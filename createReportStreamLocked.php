...
if($isDoctor==1){
    ...
    if($EntryTypegroup==10)
    {
        $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 and markfordelete=1 and Tipo NOT IN (select Id from tipopin where Agrup=9) AND (".$life_query." OR hide_from_member is null)  ORDER BY Fecha DESC ".$limitString);
    ...
    {
        $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 and markfordelete=1 and Tipo NOT IN (select Id from tipopin where Agrup=9) AND (".$life_query." OR hide_from_member is null)  ".$limitString." ORDER BY Fecha DESC LIMIT ".$limit);
        $query->bindValue(1, $queUsu, PDO::PARAM_INT);
        // $query->bindValue(2, $MedID, PDO::PARAM_INT);
    }

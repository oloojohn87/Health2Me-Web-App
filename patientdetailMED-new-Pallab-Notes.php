...
<!--  <body onload="$('.note').trigger('click'); $('.TABES').children().trigger('click');"> -->
  <body onload=" $('.TABES:eq(<?php echo $tabToOpen;?>)').click();">
    ...
    <button id="modal_phoneNotes" data-target="#header-phoneNotes" data-toggle="modal" class="btn btn-warning" style="display: none;">Modal with Header</button> 
         ...
         <div class="modal-body" style="text-align: center;font: 14pt Arial, sans-serif; background: lightgrey;">
			<center>
				<table style="background:transparent; height:150px;" >
					...
					<tr>
						<td style="height:24px;">Note : </td>
						<td style="height:24px;"><textarea rows="5" cols="150" style="width:420px;resize:none" id="note_text">	</textarea></td>
					</tr>
				</table>
			</center>
		 </div>
         ...
         <div class="modal-footer">	
			 <a href="#" class="btn btn-success" data-dismiss="modal" id="AddNote">Add Note</a>
	         <a href="#" class="btn btn-primary" data-dismiss="modal" id="CloseNote">Close</a>
         </div>
      </div>  
	...
	<!-- image upload changes end-->
        ...
        <input type="hidden" id="IdUsuP" value="<?php echo $IdUsu ?>" />
        	...
        	<div class="grid-content overflow">
			  ...
			  <div class="pull-left" style="margin-left:20px;"><a href="medicalPassport.php?IdUsu=<?php echo $IdUsu ?>" class="btn" target="_blank"><i class="icon-compass"></i> SUMMARY</a> </div>
			  <div id="BotonRecords" style="margin-left:20px;" data-target="#header-modal4" data-toggle="modal" class="pull-left"><a href="#" class="btn" title="Records Board"><i class="icon-desktop"></i> Board</a> </div>
			 ...
			 <!-- <div id="Telemedicine" style="margin-left:20px;" class="pull-left"><a class="btn" title="Telemedicine"><i class="icon-camera"></i> Telemedicine</a> </div> -->
              ...
              <!-- Adding below Notes button for taking notes by doctor during telemedicine phone call -->
  ...
  <!--  <script type="text/javascript" src="http://code.jquery.com/jquery-1.8.0.min.js"></script>--->
    ...
    $(document).ready(function() {
        ...
        if(telemedOn)
        {
            var webrtc = new SimpleWebRTC({localVideoEl: 'localVideo',remoteVideosEl: 'remoteVideo',autoRequestMedia: true, url: 'http:8888'});
    ...
    //Start of ajax call for adding notes using AddNote button    
        $('#AddNote').live('click',function()	
            {
                var userId = <?php echo $IdUsu ?>;
                var docId = <?php echo $IdMed ?>;
                var date = $('#note_date').val();
                var text = '"'+$('#note_text').val()+'"';
                
                $.post("addPatientNotes.php",{notes: text,date: date,doctorId: docId,patientId: userId},function(data,status)
                       {
                        
                           console.log(text);
                           alert("This is text sent:"+data);
                       });
   ...
   $("#StreamContainerALL").scroll(function() {
	...
	if( event.originalEvent !== undefined)
	{
        if(clicks === 1) {
			...
			//commented below line by Pallab as there was coming undefined label near detailed report tracking history

...
$result->bindValue(1, $IdMEDEmail, PDO::PARAM_STR);
$result->execute();

$row = $result->fetch(PDO::FETCH_ASSOC);


/*$result = mysql_query("Select * from doctors where IdMEDEmail = '$IdMEDEmail'"); 
...
$result = $con->prepare("UPDATE doctors SET  token = ? WHERE id = ?");
...
$confirm_button = '<a href='.$domain.'/ResetPassword.php?token='.$confirm_code.' style="cursor:auto; color:#ffffff;display:inline-block;font-family:\'Helvetica\',Arial,sans-serif;width:auto;white-space:nowrap;min-height:32px;margin:5px 5px 0 0;padding:0 22px;text-decoration:none;text-align:center;font-weight:bold;font-style:normal;font-size:15px;line-height:32px;border:0;border-radius:4px;vertical-align:top;background-color:#3498db" target="_blank"><span style="display:inline;font-family:\'Helvetica\',Arial,sans-serif;text-decoration:none;font-weight:bold;font-style:normal;font-size:15px;line-height:32px;border:none;background-color:#3498db;color:#ffffff">Yes, reset my password.</span></a>';

$adicional ='<p>Please follow the link to verify your identity and complete the password reset process that was requested.</p><p>'.$info_block.'</h3></span><p></span></p><p>Please use your Email ID for sign in purposes.</p>';

$Sobre = $Tema;
//$Body = '<p>Thanks for your interest in our services!</p><p>Please click on the following link to activate password reset for your account.</p><p>'.$domain.'/resetPassword.php?token='.$confirm_code.'</p>'.$adicional;

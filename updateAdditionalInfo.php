...
$iddoctor = ($_SESSION['MEDID'] == $_SESSION['BOTHID'] ? $_SESSION['MEDID'] : 0);
...
$checkRow = $con->prepare("SELECT * FROM basicemrdata WHERE IdPatient=? ");
$checkRow->bindValue(1, $idpatient, PDO::PARAM_INT);
$resultRow = $checkRow->execute();

if($checkRow->rowCount() > 0){

$queryText = $con->prepare("Update basicemrdata SET insurance=?, address=?,City=?, state=?, zip=?,latest_update=NOW(),doctor_signed=?,DOB=?,bloodType=?,weight=?,weightType=?,height1=?,height2=?,heightType=?  where IdPatient=? ");
...
//		  echo $error;
    $queryText = $con->prepare("INSERT INTO basicemrdata SET IdPatient=?,insurance=?,address=?,City=?, state=?, zip=?,latest_update=NOW(),doctor_signed=?,DOB=?,bloodType=?,weight=?,weightType=?,height1=?,height2=?,heightType=?");

  ...
  <body onload="$('#BotonBusquedaPac').trigger('click');" style="background: #F9F9F9;">
    	...
    	<input type="hidden" id="IdMEDLogo" Value="<?php echo $MedUserLogo; ?>">	
		   ...
		   <div class="btn-group pull-right" >
            ...
            <ul class="clear_ul" >
              ...
              <li><a id="DropBoxID"><i class="icon-cloud"></i>Add Box</a></li>
	...
	<div id="content" style="background: #F9F9F9; padding-left:0px;">
         ...
         <div class="modal-body" id="ContenidoModal" style="height:320px;">
			 <div style="margin:20px;">Date of the Report: <input type="text" name="repdate" id="datepicker1" style="font-size:24px;"></div>
     ...
     <ul class="menu-speedbar">
		 ...
		 <li><a href="dashboard.php" class="act_link" >Dashboard</a></li>
     ...
     <div class="content" style="height:3500px;">
	   ...
	   <div class="grid" class="grid span4" style="width:1100px; margin: 0 auto; margin-top:30px;">
		...
		<!--		 <a href="billing_test.php" class="btn" title="Billing" style="color:black; margin-right:20px;  float:right;"><i class="icon-plus"></i>Billing</a> -->
			     <a href="dropzone.php" class="btn" title="Patient Creation" style="color:black; margin-right:20px;  float:right;"><i class="icon-plus"></i> Patient Creation</a> 
	...
	*/	  			/*KYLE	
						...
						$G6=($EstadCanal[7] * $maximoR) / $maximo;
		  	    		...
		  	    		<div style="height:40px; width:80px; float:left;  text-align:center; margin-left:48px;">
		  	    		<p style="font-family: arial; font-size:18px; color:<?php echo $C6.'0.99)' ?>; ">Cloud</p>
...
$('#datepicker1').datepicker({
	...
	function changeBoxText(){
		var cadena = '<?php echo $domain ;?>/CheckBoxAccount.php?docid=<?php echo $MedID ?>';
		var RecTipo = LanzaAjax (cadena);
		if(RecTipo==1){
			 $("#DropBoxID").html('<i class="icon-cloud"></i>Delink Box.com');
		}else{
			
			$("#DropBoxID").html('<i class="icon-cloud"></i>Link Box.com');
		}			
	}
	
	changeBoxText();
		
    $("#DropBoxID").live('click',function(){
				//var uniqueID="";
				var cadena = '<?php echo $domain ;?>/CheckBoxAccount.php?docid=<?php echo $MedID ?>';
				var RecTipo = LanzaAjax (cadena);
				
				if(RecTipo==1){
					 del=confirm("Are you sure to revoke the Box account.Press OK to confirm");
					 if(!del)
						return;
					else{
					 
						var cadena = '<?php echo $domain ;?>/BOX/BoxChannelClientRevoke.php?Idmed=<?php echo $MedID ?>';
						var RecTipo = LanzaAjax (cadena);
						alert(RecTipo);
					 }
				changeBoxText();
				...
				window.open('<?php echo $domain;?>/BOX/BoxChannelAuth.php?uniqueID=<?php echo $MedID ?>','','left=200,width=1000,height=600,resizable=0,scrollbars=1');

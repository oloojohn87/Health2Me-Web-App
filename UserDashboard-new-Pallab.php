...
<input type="hidden" id="UserHidden">
	...
	<!-- Start of Modal window for Send button -->
    
    <!-- End of modal window for Send button -->
...
<div id="modalContents" style="display:none; text-align:center; padding-top:20px;">
		...
		<a id='ButtonWeemo'  value="Telemedicine" class="btn" title="Telemedicine" style="text-align:center; padding:15px; width:40px; height:40px; color:#22aeff; margin-left:20px; float:left;"> 
             ...
             <div style="float:left; height:245px; width:900px; margin: 30px auto; margin-top:-10px; margin-left:30px; margin-right:30px; padding:10px; border:1px solid #cacaca; font-size:0px; ">             
                ...
                <div style="float:left; margin-top:-40px; width:500px; border:1px solid #cacaca; padding:10px; margin:10px; border-radius: 3px;">
             		<div style="margin-left:-10px; margin-top:-10px; margin-bottom:-10px; float:left; width:60px; height:222px; border-radius:0px;  border-top-left-radius: 2px; border-bottom-left-radius: 2px; background-color:#54bc00;">
             			<div style="width:180px;height:50px; position:relative; top:80px; left:-50px; -ms-transform:rotate(270deg); -moz-transform:rotate(270deg); -webkit-transform:rotate(270deg);-o-transform:rotate(270deg); color:white; font-size:24px;">Health Passport</div>
	             		 </div>


             	    <canvas id="myCanvas" width="200" height="200" style="float:left; border:1px solid #cacaca; margin-left:20px; cursor: alias; "></canvas>
					<canvas id="myCanvas2" width="200" height="200" style="float:left; border:1px solid #cacaca; margin-left:20px; cursor: alias;"></canvas>
				</div> 
                <div style="float:left; margin-top:-40px; width:310px; height:200px; border:1px solid #cacaca; padding:10px; margin:10px; border-radius: 3px;">
         ...
         <!-- Start of Pending Mail-->
    <div class="grid" class="grid span4" style="margin:0px auto; margin-top: 10px; margin-left:10px; margin-right:10px; padding:20px; padding-top: 10px;display:none;" id="pendingMail">
        <div style="margin-top:10px;"><span class="label label-info" id="EtiTML" style="background-color:#22aeff; margin:10px; margin-left:0px; margin-bottom:20px; font-size:16px; text-shadow:none; text-decoration:none;">Pending Mails</span></div>
        <table style="width: 100%; background-color: #FFFFFF;" id="pending_mail">
        </table>
    </div>
    <!-- End of Pending Mail --> 
  ...
  <?php
     ...
     while ($n<10)
     {
        //echo '<i class="'.$TipoIconGroup[$n].'" style="color:'.$TipoColorGroup[$n].';"></i>';
        echo '<i class="'.$TipoIconGroup[$n].'" style="color:white; visibility:hidden;"></i>';
	    $n++;
     }

  ?>		
<img id="Imaging" src="images/icons/badge_imaging_svg.png" style="visibility:hidden;" >
<img id="Reports" src="images/icons/badge_document_svg.png" style="visibility:hidden;" >
<img id="Laboratory" src="images/icons/badge_lab_svg.png" style="visibility:hidden;" >
    ...
    //setInterval(EchoPendingMail,3000);
   
    var num_pending_review_mails = 0;
    
    function EchoPendingMail()
    {
     
        var userId = <?php echo $UserID; ?>;
        
        $.post("getPendingMail.php", {Patient: userId}, function(data, status)
		{
            var items = JSON.parse(data);
            var num = items.length;
            
            var doctorId = $("#EmailIDRequestPage").val();
            ...
            for(var i = 0; i < items.length; i++)
            {
                ...
                html += '<span style="color: #494949;">' + items[i].CONTENT + ' </span>';
                html += '<span style="color: #22aeff;">' + doctorId + '</span></div></td>';
                    
                html += '<td style="width: 20%; padding:6px; ">';
                html += '<a id="pending_review_mail" href="javascript:void(0)" class="btn" title="Read" style="width:65%; color:grey; margin-bottom: 2px;">Review</a></td></tr>';
            }
            html += '</tbody>';
            $('#pending_mail').html(html);
            if (num > 0)
            {
                $('#pendingMail').slideDown();
            }
        });
    }
	...
	var timerId = setTimeout(function() { 
	  ...
	  while (n < 7)
	  {
	      ...
	      */
		  ...
		  console.log ('A: '+ n +' - '+ midAngle+' SizSeg = '+SizSeg[n]);
	  ...
	  var sources = {
			  Admin: 'images/icons/Admin_svg.png',
			  history: 'images/icons/history_svg.png',
			  medication: 'images/icons/medication_svg.png',
			  immuno: 'images/icons/immuno_svg.png',
			  family: 'images/icons/family_svg.png',
			  habits: 'images/icons/habits_svg.png'
	  };

var arr = $.parseJSON(sources);
/*
	  var sources = [
			   ["setup","Admin_svg.png"],
			   ["history","history_svg.png"],
			   ["medication","medication_svg.png"],
			   ["immuno","immuno_svg.png"],
			   ["family","family_svg.png"],
			   ["habits","habits_svg.png"]
	  ];
*/		   
	  n = 1;
	  loadImages(sources, function(images) {
			  	  ...
			  	  while (n < 7)
			  	  {
					  ...
					  var posy = y + (radius * Math.sin (midAngle)) - 10;
				  	  ...
				  	  console.log ('B: '+ n +' - '+ midAngle+' SizSeg = '+SizSeg[n]);
				  	  
				  	  var whatImg = images.Admin;
				  	  //var whatImg = images[n][1];
				  	  console.log ('element = '+whatImg);
				  	  context.drawImage(whatImg,posx,posy,20,20);
      ...
      var counterClockwise = false;
 	  ...
 	  var ColSegR = new Array();
	  var SizSegR = new Array();
	  var ImgSegR = new Array();
	  ...
	  ColSegR[3] = '#22aeff';
	  SizSegR[3] = 2;
	  ImgSegR[3] = 'Laboratory';
     
	  var MaxSegments = 3;
	  var MaxReports = 12;
     
      contextR.lineWidth = widthSegment;
	  startAngle = 0 - (Math.PI/2);
	  ...
	  while (n <= MaxSegments)
	  {
	      ...
	      // Draw the segments
	      contextR.strokeStyle = SegColor;
	      endAngle = startAngle + TranslateAngle(SizSegR[n],MaxReports);
	      midAngle = startAngle + (TranslateAngle(SizSegR[n]/2,MaxReports));
		  contextR.lineWidth = widthSegment;
	      contextR.arc(x, y, radius, startAngle, endAngle, counterClockwise);
	      contextR.stroke();
		  ...
		  var imageObj=document.getElementById(ImgSegR[n]);
		  contextR.drawImage(imageObj, posx, posy,SizeBadge,SizeBadge);
		  ...
		  contextR.fillStyle = '#54bc00';
		  contextR.textAlign = 'left';
		  contextR.textBaseline = 'top'; // important!
		  contextR.font = font;
		  var w = contextR.measureText(message).width;
		  var TextH = GetCanvasTextHeight(message,font);
		  contextR.fillText(message, x-(w/2), y-(TextH));

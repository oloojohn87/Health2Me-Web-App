...
$getDoctors = $con->prepare("SELECT distinct(receiver_id) as doctorIds FROM message_infrastructureuser WHERE patient_id = ? && subject = 'Send Mail' and content = 'Mail From Patient'");
$getDoctors->bindValue(1, $UserID, PDO::PARAM_INT);
$getDoctors->execute();


$emailIdOfDoctors = array();

while($doctor = $getDoctors->fetch(PDO::FETCH_ASSOC))
     {
     
        $getDoctorEmail = $con->prepare("select IdMEDEmail from doctors where id = ?");
        $getDoctorEmail->bindValue(1,$doctor['doctorIds'],PDO::PARAM_INT);
        $getDoctorEmail->execute();
        $email = $getDoctorEmail->fetch(PDO::FETCH_ASSOC);
        array_push($emailIdOfDoctors,$email['IdMEDEmail']);
     }
...
<div id="modal_send" title= "Send Reports Link" style="display:none; text-align:center; padding:10px;">
        ...
        <div id ="doctorsSend" style="margin:auto;display:none;width:206px;height:100px;margin-bottom:-100px;background-color:#45D682;z-index:5;position:relative;">
            <?php echo $emailIdOfDoctors[0]; ?>
        </div>
    
        <!-- add stream here -->
        <div style="width: 95%; margin: auto; height: 300px; overflow: scroll; text-align: center; margin-top: 30px; margin-bottom: 20px;z-index:4;position:relative;" id="sendStreamContainer">
        ...
        <button id="CaptureEmail2Send2Doc" style="border:0px;border-radius:6px;height: 24px; width:50px; color:#FFF; background-color:#22aeff;float:bottom; margin-top:20px;z-index:3;position:relative;" lang="en">Send</button>
    
   <!--  <button id="getPreviousDoctors" style="border:0px;border-radius:6px;height: 24px; width:50px; color:#FFF; background-color:#22aeff;float:bottom; margin-top:20px;" lang="en">Select Previous Doctors</button> -->
</div>
            ...
            $.post("EditUserFam.php", {Name: $("#family_member_name").val(), Surname: $("#family_member_surname").val(), DOB: $("#family_member_dob").val(), Phone: $("#family_member_phone").val(), Password: $("#family_member_password").val(), Password2: $("#family_member_password2").val(), Email: $("#family_member_email").val(), Order: $("#family_member_order").val(), Gender: $("#family_member_gender").val(), Relationship: $("#family_member_relationship").val(), Admin: $("#give_admin_privileges").children().eq(0).val(), Owner: $("#USERID").val(), Mode: family_editing_mode, User: selected_family_member}, function(data, status)
            {
                if(data.substr(0, 4) == 'GOOD')
                {
                    ...
                    if(family_editing_mode == 1)
                    {
                        ...
                        $("#family_members_dropdown").append('<li><a href="#" class="change_user_dropdown_button" id="user_'+items[1]+'_'+$("#family_member_email").val()+'_'+items[2]+'" lang="en"><i class="icon-user"></i> '+$("#family_member_name").val()+' '+$("#family_member_surname").val()+'</a></li>');
                        ...
                        html = '<button id="user_'+items[1]+'_'+$("#family_member_email").val()+'_'+items[2]+'" class="user_button" style="width: 100%; height: 30px; background-color: #22AEFF; outline: 0px; border: 1px solid #555; color: #FFF; border-bottom-left-radius: 5px; border-bottom-right-radius: 5px;">'+$("#family_member_name").val()+' '+$("#family_member_surname").val()+'</button>';
                    ...
                    else
                    {
                        ...
                        $('button[id^="user_'+selected_family_member+'"]').attr('id', 'user_'+selected_family_member+'_'+$("#family_member_email").val()+'_'+items[2]);
                        
                        $('a[id^="user_'+selected_family_member+'"]').html('<i class="icon-user"></i> '+$("#family_member_name").val()+' '+$("#family_member_surname").val());
                        $('a[id^="user_'+selected_family_member+'"]').attr('id', 'user_'+selected_family_member+'_'+$("#family_member_email").val()+'_'+items[2]);
...
//	$('#UpdateSumm').live('click',function(){
    ...
    $('#SetUp').live('click', function()
    {
        ...
        $("#setup_modal").dialog({bigframe: true, width: 650, height: 400, resize: false, modal: true});
...
//Start of code inserted from userdashboard-new-pallab.php
    ...
    //New code for getting previous doctor mails
        $("#EmailID").focus(function(){
        
           $("#doctorsSend").css("display",'block');
        });
        $("#EmailID").focusout(function(){
        
           $("#doctorsSend").css("display",'none');
        });

        
        var doctorEmails;
        var count = <?php echo sizeof($emailIdOfDoctors); ?>
        
        doctorEmails = <?php echo "'".json_encode($emailIdOfDoctors)."';"; ?>
        var email = $.getJSON(doctorEmails);
      ...
      /*  while(count >= 0)
        ...
        console.log("count is "+count+" Emails of Doctors   "+email[0]);

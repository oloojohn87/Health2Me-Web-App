...
if($isDoctor==1){
    ...
    $q=	$con->prepare("select * from ".$dbname.".lifepin LP INNER JOIN ((select distinct(B.idDoctor) from ".$dbname.".doctorsgroups B INNER JOIN (select A.idGroup,A.idDoctor from ".$dbname.".doctorsgroups A where A.idDoctor=?) C where B.idGroup=C.idGroup) UNION (select DLD.IdMED from ".$dbname.".doctorslinkdoctors DLD INNER JOIN (select distinct(B.idDoctor) from ".$dbname.".doctorsgroups B INNER JOIN (select A.idGroup,A.idDoctor from ".$dbname.".doctorsgroups A where A.idDoctor=?) C where B.idGroup=C.idGroup) DG where DG.idDoctor=DLD.IdMED2 and IdPac=? and estado=2)) SQ WHERE (SQ.idDoctor=LP.IdMED)  and LP.IdUsu=? and (LP.markfordelete=0 or LP.markfordelete is null) and (LP.IsPrivate=0 or LP.IsPrivate is null) and NOT (Tipo IN (select Id from tipopin where Agrup=9) and LP.IdMED!=?) and emr_old=0 ".$tipoString." ORDER BY Fecha DESC LIMIT ".$offset.','.$limit);
    ...
    $q=	$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and NOT (Tipo IN (select Id from tipopin where Agrup=9) and IdMED!=?) and emr_old=0 ".$tipoString." ORDER BY Fecha DESC LIMIT ".$offset.','.$limit);

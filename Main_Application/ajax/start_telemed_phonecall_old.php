...
if(isset($_POST['pat_phone']))
{
    ...
    $pat_name = str_replace('_', ' ', $pat_name);
...
if($pat_row['GrantAccess'] == 'HTI-RIVA'){
	$doc_phone = "+18448008801";
	$doc_name = "Riva Call Center";
	$doc_id = 1;
$override_doctor_validation = true;
}

if($pat_row['GrantAccess'] == 'HTI-COL'){
	$doc_phone = "+18779669066";
	$doc_name = "HTI Call Center";
	$doc_id = 1;
$override_doctor_validation = true;
}

// check if the doctor is in a video consultation
if($row['in_consultation'] != 1 || $override_doctor_validation)
{
    ...
    $consults = $con->prepare("SELECT consultationId FROM consults WHERE Status = 'In Progress' AND Doctor = ?");
    $consults->bindValue(1, $doc_id, PDO::PARAM_INT);
    $consults->execute();
    $in_consults = $consults->rowCount();
    
    // check if the doctor is in a phone consultaiton
    if((!in_array(intval($row['id']), $docs_in_consultation) && $in_consults == 0) || $override_doctor_validation)
    {
        ...
        if($has_credit_card || $pat_row['GrantAccess'] == 'HTI-COL'|| $pat_row['GrantAccess'] == 'HTI-RIVA' || $override)
        {
            $date = date_create();
            ...
            elseif($pat_row['GrantAccess'] == 'HTI-RIVA')
            {
                $contract = 'HTI-RIVA';
            }
	    else
	    ...
	    if($doc_id == 1){
            $results = $con->prepare("INSERT INTO consults(contract,DateTime,status,type,Patient,Doctor,doctorName,doctorSurname,patientName,patientSurname,lastActive,description, Doctor_Status) values(?,NOW(),'In Progress','phone',?,?,?,?,?,?,NOW(),".$desc.",1)");
            ...
            if(strlen($description) > 0)
            {
                $results->bindValue(8, $description, PDO::PARAM_STR);
            }
            $results->execute();

	    }else{
		$results = $con->prepare("INSERT INTO consults(contract,DateTime,status,type,Patient,Doctor,doctorName,doctorSurname,patientName,patientSurname,lastActive,description) values(?,NOW(),'In Progress','phone',?,?,?,?,?,?,NOW(),".$desc.")");
            $results->bindValue(1, $contract, PDO::PARAM_STR);
            $results->bindValue(2, $pat_id, PDO::PARAM_INT);
            $results->bindValue(3, $doc_id, PDO::PARAM_INT);
            $results->bindValue(4, $doc_firstname, PDO::PARAM_STR);
...
<?php if(isset($_REQUEST["Digits"]) || $_GET["grant_access"] == "riva" || $_GET["grant_access"] == "hti"){
require("../environment_detailForLogin.php");
$dbhost = $env_var_db["dbhost"];
$dbname = $env_var_db["dbname"];
$dbuser = $env_var_db["dbuser"];
...
$result = mysql_query("SELECT in_consultation FROM doctors where id='.$doc_id.'");
$row = mysql_fetch_array($result);$count = $row["in_consultation"];
/*if($count == "2"){
...
$result = mysql_query("UPDATE doctors SET in_consultation=2 WHERE id='.$doc_id.'");
            ...
            $doc_url = 'http://'.$_SERVER['HTTP_HOST'].'/live/phone_appointment_ask.php?pat_name='.str_replace(" ", "%20", $pat_name).'&grant_access='.$contract.'&conference_id='.$conference_id;
            $pat_url = 'http://'.$_SERVER['HTTP_HOST'].'/live/phone_appointment_wait.php?info='.$doc_id.'-'.str_replace(" ", "%20",$doc_name).'-'.$conference_id.'-'.$pat_id.'-'.$recent_id.'&grant_access='.$contract;
			...
			if($pat_row['GrantAccess'] != 'HTI-RIVA' && $pat_row['GrantAccess'] != 'HTI-COL'){
			sleep(45);
			}
			
			$check_accepted = $con->prepare("SELECT * FROM consults WHERE consultationId = ?");

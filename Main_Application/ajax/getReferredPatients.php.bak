...
$groupname=isset($ArrayDoctors[$cnt])?$ArrayDoctors[$cnt]:"";
//echo 'userid '.$userid;
$result = $con->prepare("SELECT * FROM doctorslinkdoctors WHERE IdMED = ? ORDER BY IdMED2 DESC");
...
$cadena='';*/
	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
		...
		//Add changes for showing group details
		//$resdetails=mysql_query("select idGroup from doctors where id ='$seekthis'");
		$resdetails=$con->prepare("select idGroup from doctorsgroups where idDoctor =?");
		$resdetails->bindValue(1, $seekthis, PDO::PARAM_INT);
		$resdetails->execute();
		
		//echo '*************  ';
		//echo "select idGroup from doctors where id ='$seekthis'";
		//echo '  *************  ';
		
		$groupid="";
		while($row12 = $resdetails->fetch(PDO::FETCH_ASSOC)){
			$groupid=$row12['idGroup'];		
		}

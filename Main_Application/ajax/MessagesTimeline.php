					...
					if (NumReports>0)
					{
						MesTimeline = MesTimeline + '<div style="margin:0 auto; width:95%; border:solid #cacaca; border-radius:5px; height:80px;">'; 
							...
							while (rn < NumReports)
							{
								...
								MesTimeline = MesTimeline + '<img style="height:70px; margin:5px; border:solid 1px #cacaca;" src="temp/'+<?php echo $MedID;?>+'/PackagesTH_Encrypted/'+thumbnail+'">';
								rn++;
							}
						MesTimeline = MesTimeline + '</div>'; 
					}	
...
/*	   	window.open('printimage.php?path='+path+'&IdUs=<?php echo $IdUsu ?>&MedID=<?php echo $Idmed?>','','left=200,width=900,height=700,resizable=0,scrollbars=1');
        ...
        if(clicks === 1) {
               ...
               timer = setTimeout(function() {
				...
				else if(extensionR=='jpg' || extensionR=='jpeg' )
				{
					...
					var conten = '<img id="ImageN" style="border:1px solid #666CCC; margin:0 auto; display:block;max-height:1500px;max-width:600px;"  src="'+contenURL+'" alt="'+queId+'">';

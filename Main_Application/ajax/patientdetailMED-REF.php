...
$q->bindValue(1, $IdMed, PDO::PARAM_INT);
$q->execute();
...
$result = $con->prepare("SELECT * FROM doctorslinkdoctors where Confirm=?");
...
if($countA==1){
	...
	if($estado==1){
		...
		$q = $con->prepare("UPDATE doctorslinkdoctors SET Confirm = '***R****' , FechaConfirm = NOW(), estado = '2' WHERE id =? ");
		$q->bindValue(1, $IdEntry, PDO::PARAM_INT);
		$q->execute();
		
	
		$h = $con->prepare("insert into referral_stage set referral_id=?,stage=1");
		$h->bindValue(1, $IdEntry, PDO::PARAM_INT);
		$h->execute();

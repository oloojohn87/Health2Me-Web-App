...
while ($rowPRE = $resultPRE->fetch(PDO::FETCH_ASSOC)) {
    ...
    $PassPac = $rowPRE['IdUsRESERV'];
        ...
        $dluPIN=$con->prepare("select * from ".$dbname.".doctorslinkusers where IdUs=? and IdMED=?");
       ...
       //echo $dluqquery;
        ...
        if($viewablePIN->fetch(PDO::FETCH_ASSOC)){
           ...
           // echo 'countPIN '.$countactualPIN;
            ...
            if($hasfullaccess==1){
                      $viewable=$con->prepare("Select count(*) as patreport from ".$dbname.".lifepin where (IdMed IS NULL or IdMed=0 or IdMed=?) and (markfordelete IS NULL or markfordelete=0) and IdUsu=?");
	    ...
	    echo '<tr onclick="setAppointmentForMember('.$rowPRE['Identif'].', '.$room_id.');" class="CFILA" id="'.$rowPRE['Identif'].'"><td>'.$rowPRE['IdUsFIXED'].'</td>';
		echo '<td>'.$UserName.'</td>';
		echo '<td style="font-weight:bold;"><i class="fam-resultset-next"></i>'.$UserSurname.'  '.$medalla.'</td>';

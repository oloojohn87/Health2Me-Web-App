...
{//$probeFlag = true;
    ...
    if(isset($_GET['filter'])){
        ...
        (SELECT DISTINCT patientID, creationDate FROM probe WHERE doctorPermission = 1 AND scheduledEndDate >= SYSDATE() AND doctorID = :idDocforTrkUp GROUP BY patientID) P ON DL.id = P.patientID"; 
        
        if($_GET['filter'] == 'prbUp') $sql .= " INNER JOIN
        (SELECT DISTINCT patientID, creationDate FROM probe WHERE doctorPermission = 1 AND scheduledEndDate >= SYSDATE() AND (creationDate BETWEEN SYSDATE()-INTERVAL 30 DAY AND SYSDATE()) AND doctorID = :idDocforPrbUp GROUP BY patientID) P ON DL.id = P.patientID";
...
//while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    $result5 = $con->prepare("SELECT * FROM proberesponse WHERE probeID = ? AND (responseTime BETWEEN SYSDATE()-INTERVAL 30 DAY AND SYSDATE())");
    $result5->bindValue(1, $row6['probeID'], PDO::PARAM_INT);
    $result5->execute();

    $count5 = $result5->rowCount();
    //echo '  ********************** COUNT = '.$count2;
    $Totprobe = $Totprobe + $count5;
    ...
    if($num_alerts > 0)
    {
        ...
        $probe_icon = '<div id="ALERT_'.$PatientId.'_'.$alert['id'].'_'.$alert['created'].'" class="CreateProbe patient_row_button '.$new_probes.'" style="background-color: #D84840; color: #FFF; border-color: #D84840; -webkit-animation: glow 1.5s linear infinite;">
                <i id="'.$PatientId.'"  class="icon-signal"></i></div>';
    }
    else if($Totprobe > 0)
    {
        // probe responses received
        $probe_icon = '<div id="'.$PatientId.'" class="CreateProbe patient_row_button '.$new_probes.'" style="background-color: #22AEFF; color: #FFF; border-color: #22AEFF;">
                <i id="'.$PatientId.'"  class="icon-signal"></i></div>';
    }
    else if($num1>=1 && ($row6['patientPermission'] == 0 || $row6['doctorPermission'] == 0))
    {
        // pending
        $probe_icon = '<div id="'.$PatientId.'" class="CreateProbe patient_row_button '.$new_probes.'" style="background-color: #EAEAEA; color: #22AEFF; border-color: #CCC;">
                <i id="'.$PatientId.'"  class="icon-signal"></i></div>';
    }
    else if($num1>=1 && $row6['patientPermission'] == 1 && $row6['doctorPermission'] == 1)
    {
        // activated
        $probe_icon = '<div id="'.$PatientId.'" class="CreateProbe patient_row_button '.$new_probes.'" style="background-color: #96CD6A; color: #FFF; border-color: #96CD6A;">
                <i id="'.$PatientId.'"  class="icon-signal"></i></div>';
    }
    else
    {
    ...
    echo '<div class="doctor_row_wrapper">
            ...
            -->
                ...
                <div style="width: 850px; margin: auto;">
                    <div class="external_graph_container" id="probe_graph_'.$row6['probeID'].'_1"></div>

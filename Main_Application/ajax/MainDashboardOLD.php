...
function HideTWWait(){
	...
	$('#BotonRef').bind('click mousedown keydown', function(event) {
		...
		var n = 0;
		NewPatientData = '';
		NewPatientData = NewPatientData + '<span class="label label-info" id="EtiTML" style="margin:10px; margin-left:0px; margin-bottom:20px; font-size:16px; text-shadow:none; text-decoration:none;">Your recent EMR activity</span>';
		NewPatientData = NewPatientData + '<div style="width:100%; margin-bottom:20px;"></div>';

		while (n < Pa)
		{
			NewPatientData = NewPatientData + '<a href="patientdetailMED-new.php?IdUsu='+PatData[n].id+'" style="text-decoration:none;"><p style="margin-bottom:0px;"><span style="color: #22aeff; font-size:16px; margin-right:10px;">' +n+' <i class="icon-caret-right"></i></span><span style="color: #54bc00; font-size:16px;">'+PatData[n].name+' '+PatData[n].surname+'</span></p></a>';
			n++;
		}
	    if (NewPatientData > '')
        {

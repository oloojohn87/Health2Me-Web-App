...
/* Below variables as used orginally in patientdetailmed-new.php
...
//Below variables were added to make original patientdetailmed-new.php work for patientdetailmed-new-forsend.php as requirement for send button
$NombreEnt = $_GET['Nombre']; 
$PasswordEnt = '';//$_SESSION['Password'];
$Acceso = $_GET['Acceso'];
$IdUsu = $_GET['IdUsu'];
...
//Below temp folders are created where TH and data would be used to copy/write. This step differs from original patientdetailMED-new.php and originally these folders are created in login.php page

if (!file_exists('temp/'.$IdMed)) 
{
    mkdir('temp/'.$IdMed, 0777, true);
	mkdir('temp/'.$IdMed.'/Packages_Encrypted', 0777, true);
	mkdir('temp/'.$IdMed.'/PackagesTH_Encrypted', 0777, true);
}
...
if($referral_stage==2){
	...
	if($attachments_dld!=0){
		...
		while($cntt>0){
		$getinfo = mysql_query("select id from bpinview USE INDEX(I1) where viewIdUser='$USERID' and viewIdMed='$doc_id' and content='Report Viewed' and IDPIN='$report_id[$i]'");
		...
		$getinfo = mysql_query("select id from bpinview USE INDEX(I1) where viewIdUser='$USERID' and viewIdMed='$doc_id' and content='Report Viewed'");
		$cnt_info=mysql_num_rows($getinfo);
		//echo "".$cnt_info."<br>";
		if($cnt_info>3)
		{
...
if($multireferral==1){
	...
	if($referral_stage_array[$i]==2){
		...
		if($attachments_dld_array[$i]!=0){
			...
			//Remember to add the check for date of the reports viewed. It should always be greater than appointment schedule date
			while($cntt>0){
			$getinfo = mysql_query("select id from bpinview USE INDEX(I1) where viewIdUser='$USERID' and viewIdMed='$doc_id_array[$i]' and content='Report Viewed' and IDPIN='$report_id[$j]'");
			$cnt_info=mysql_num_rows($getinfo);
			if($cnt_info)
				$reportviewed=true;
			else
			...
			$getinfo = mysql_query("select id from bpinview USE INDEX(I1) where viewIdUser='$USERID' and viewIdMed='$doc_id_array[$i]' and content='Report Viewed'");
			$cnt_info=mysql_num_rows($getinfo);
			//echo "".$cnt_info."<br>";
			if($cnt_info>3)
			{
...
if($multireferral>0)
{
	...
	$count=0;
	$query = "SELECT attachments FROM doctorslinkdoctors where Idpac ='$IdUsu' and (IdMED='$IdMed' or IdMED2='$IdMed') ";
  ...
  <body onload=" $('.TABES:eq(<?php echo $tabToOpen;?>)').click();">
	...
	<div class="header" >
		   ...
		   <div class="btn-group pull-right" >
            ...
            <ul class="clear_ul" >
           <!-- Commenting out the below two lines for for the Send button implementation for Non H2M doctor -Pallab -->
            <!--   <li><a href="signin.html"><i class="icon-globe"></i> Home</a></li> -->
              
           <!--   <li><a href="medicalConfiguration.php"><i class="icon-cog"></i> Settings</a></li> -->
              <li><a href="logout-ForSendButtonUserDashboard.php?MEDID=<?php echo $IdMed;?>"><i class="icon-off"></i> Sign Out</a></li>
            </ul>
            </div>
          </div>
          <!--Button User END-->  
    ...
    <div id="videoTelemed" title="Telemedicine" style="display:none; text-align:center; width: 800px;">
        <div style="float: left; width: 25%; margin-top: 1%; margin-right: 3%;">
            <textarea cols="32" rows="15" id="telemed_notes"  style="padding: 15px; border-radius: 6px; width:100%; height: 96%; resize: none; color: #747474;  border: 1px solid #ccc;">Notes...</textarea>
        
        </div>
        <div style="float: left; width: 69%; height: 96%; margin-left: 20px; margin-top: 1%;">
            <div id="remoteVideo" style="background-color: #393939; width: 520px; height: 340px; border-radius: 6px;">
                
                
            </div>
            <div style="float:left; margin-left: 8%; margin-top: -12%;"><video id="localVideo" width="70" style="mask-image: url(small_video_mask.png);" autoplay></video></div>
	...
	<!--- VENTANA MODAL  This has been added for enabling blind report access ---> 
     ...
     <ul class="menu-speedbar">
		...
		// {
         ...
         <li><a href="logout-ForSendButtonUserDashboard.php?MEDID=<?php echo $IdMed;?>" style="color:yellow;">Sign Out</a></li>
     </ul>
   ...
   // Commenting out the below referral communication area as it is required for NOn H2M doctor
  /* if($showreferralsection!=0) {
...
*/     /*
    ...
    // else 
      {echo '<input type="hidden" id="referral_state" value="0">';} ?>
        <div class="clearfix" style="margin-bottom:20px;"></div>
...
<!--              <div class="pull-left" style="margin-left:20px;"><a href="dropzone_short.php?IdUsu=<?php echo $IdUsu ?>" class="btn"><i class="icon-th"></i> Drop Files</a> </div>
  ...
  <div id="tabsWithStyle" style="margin-top:60px" class="style-tabs"> 
          ...
          <ul id="myTab" class="nav nav-tabs tabs-main">
            <li id="0" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background: none repeat scroll 0% 0% rgb(204, 204, 204); text-align:center;"><a href="#ALL" data-toggle="tab" style="height:40px; font-size:16px;"><i class="icon-ok-sign icon-large" style="color:black; font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>ALL</a></li>
			<?
				if($multireferral>0){
					$backgroundcolor=$TipoColorGroup[5];
					echo '<li id="5" class="TABES" style="width:10%; background-color:'.$backgroundcolor.'"><a href="#ALL"   data-toggle="tab" style="height:40px; color:'.$backgroundcolor.'"><i class="icon-star icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Highlighted</a></li>';
				}
			?>
			<li id="1" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[1] ?>;"><a href="#ALL" data-toggle="tab" style=" color:<?php echo $TipoColorGroup[1] ?>;"><i class="<?php echo $TipoIconGroup[1] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Imaging</a></li>
            <li id="2" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[2] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[2] ?>;"><i class="<?php echo $TipoIconGroup[2] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Laboratory</a></li>
            <li id="3" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[3] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[3] ?>;"><i class="<?php echo $TipoIconGroup[3] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Notes</a></li>
            <li id="4" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[4] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[4] ?>;"><i class="<?php echo $TipoIconGroup[4] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Other </a></li>
            ...
            <li id="6" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[6] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[6] ?>;"><i class="<?php echo $TipoIconGroup[6] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>SUMMARY</a></li>
            <li id="7" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[7] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[7] ?>;"><i class="<?php echo $TipoIconGroup[7] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Pictures</a></li>
            <li id="8" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 11;}?>%; background-color:<?php echo $TipoColorGroup[8] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[8] ?>;"><i class="<?php echo $TipoIconGroup[8] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Pat. Notes</a></li>
            <li id="9" class="TABES" style="width:<?php if($multireferral>0) {echo 10;} else {echo 12;}?>%; background-color:<?php echo $TipoColorGroup[9] ?>;"><a href="#ALL" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[9] ?>;"><i class="<?php echo $TipoIconGroup[9] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Superbill</a></li>
	...
	<!-- Commented out by Pallab as not required by Non H2M doctor -->	
	<!-- <div class="clear"></div><br/><br/>
		...
		<span style="display:none;color:#22aeff;margin-left:100px" id="H2M_Timeline_Spin_Stream"><i class="icon-spinner icon-2x icon-spin" ></i> <b> Loading... </b> </span>
		 <i id="TLineScrollDown" class="icon-caret-down icon-2x navArrows" style="float:right;margin-right:15px;color:#54bc00"></i>
		 <i id="TLineScrollUp" class="icon-caret-up icon-2x navArrows" style="float:right;margin-right:15px;color:#54bc00;display:none"></i>
  ...
  <!-- Commented out by Pallab as not required for NOn H2M doctor -->
  <!--<div class="grid span3" style="width:90%;float:inherit">
  ...
  <!--  <script type="text/javascript" src="http://code.jquery.com/jquery-1.8.0.min.js"></script>--->
	...
	var jump=20;//used for tab scrolling
	...
	{
		$.ajax({
			url: '<?php echo $domain?>/ongoing_sessions.php?userid='+<?php echo $IdMed; ?>,
    ...
    $(document).ready(function() {
        ...
        if(telemedOn)
        {
            var webrtc = new SimpleWebRTC({localVideoEl: 'localVideo',remoteVideosEl: 'remoteVideo',autoRequestMedia: true, url: 'http://dev.health2.me:8888'});
            webrtc.on('readyToCall', function () 
            {
                ...
                $("#remoteVideo").children("video").each(function()
                 {
                    $(this).css("height", ($("#videoTelemed").css("width") * 0.6538461538 * 0.68).toString() + "px");
                    $(this).css("width", ($("#videoTelemed").css("width") * 0.68).toString() + "px");
                 });
                $.post("update_webrtc.php", {doc_id: $("#MEDID").val(), pat_id: $("#USERID").val(), status: 1, add_to_recent_doctors: 1}, function(){});
                $("#remoteVideo").children("video").each(function()
                {
                    $(this).css("height", (parseInt(ui.size.width) * 0.6538461538 * 0.68).toString() + "px");
                    $(this).css("width", (parseInt(ui.size.width) * 0.68).toString() + "px");
                });
            ...
            $("#videoTelemed").dialog({bgiframe: true, width: 800, height: 400, minWidth: 700, minHeight: 340, modal: false, 
            resize: function( event, ui ) 
            {
                if(((parseFloat(ui.size.width) * 0.68) / parseFloat(ui.size.height)) < 1.35 )
                {
                    
                    $("#remoteVideo").css("height", (parseInt(ui.size.width) * 0.6538461538 * 0.68).toString() + "px");
                    $("#remoteVideo").css("width", (parseInt(ui.size.width) * 0.68).toString() + "px");
                    $("#telemed_notes").attr("rows", (Math.floor(parseInt(ui.size.width) * 0.6538461538 * 0.68) / 22));
                    $("#remoteVideo").children("video").each(function()
                     {
                        $(this).css("height", (parseInt(ui.size.width) * 0.6538461538 * 0.68).toString() + "px");
                        $(this).css("width", (parseInt(ui.size.width) * 0.68).toString() + "px");
                ...
                {
                    $("#remoteVideo").css("height", (parseInt(ui.size.height) * 0.82).toString() + "px");
                    $("#remoteVideo").css("width", (parseInt(ui.size.height) * 0.82 * 1.529411765).toString() + "px");
                    $("#telemed_notes").attr("rows", (Math.floor(parseInt(ui.size.height) * 0.82) / 22));
                    $("#remoteVideo").children("video").each(function()
                     {
                        $(this).css("height", (parseInt(ui.size.height) * 0.82).toString() + "px");
                        $(this).css("width", (parseInt(ui.size.height) * 0.82 * 1.529411765).toString() + "px");
                     });
                }
            }, 
 ...
 $('#PrintImage').live('click',function(){
	...
	$("#BotonRecords").live('click',function() {
		...
		//{
			...
			var url = 'get_file_list.php?ElementDOM=na&EntryTypegroup=0&Usuario='+Usuario+'&MedID='+<?php echo $IdMed;?>;
			//alert(url);
			var RecTipo = LanzaAjax (url);
		//alert(RecTipo);
			list = RecTipo.split(';');
   ...
   $(".TABES").live('click',function() {
       ...
       //Commenting the below for the implementation of Send button Pallab as url changed from getNUmThumbNails to getNUmThumbnails-ForSend.php
		//var queUrl = 'getNumThumbnails.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID;
		
       var queUrl = 'getNumThumbnails-ForSend.php?Usuario='+Usuario;
				...
				$.ajax(
				   ...
				   success: function(data) {
							...
							if(RecTipo != lastRectipo)
							{
								...
								$('#ascroll').children().last().after(RecTipo);
								setTimeout(function() {highlightattachedreports();},1000);  
								//alert(lastRectipo);
  ...
  $.ajax({
	...
	function next_click()
	{
		...
		else
		{
			...
			var RecTipo = LanzaAjax (cadena);
			//alert(RecTipo);
			contenURL =   '<?php echo $domain ;?>/temp/<?php echo $IdMed ;?>/Packages_Encrypted/'+file_name;
		}   	

...
<!-- Modal -->
<div class="modal fade full-screen" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">Report View</h4>
      </div>
      <div id="ModalContent" class="modal-body" >
        <iframe id="ZoomedIframe" style="width:100%; height:100%;"></iframe>
      </div>
...
<button id="LaunchModal" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#myModal" style="display:none;">
  Launch demo modal
</button>
...
function GetTimeLine(unread)
	{
		...
		while (n < Pa)
		{
			...
			var newIndicator = '';
			var newIndicator2 = '';
			var newIndicator3 = 'rgba(34,174,255,0.1)';
			if (NewMES[n].MessageStatus == 'new' )
			{
				...
				newIndicator2 = 'border-left:10px solid #22aeff;';
				newIndicator3 = 'rgba(34,174,255,0.8)';
			}
			// NEW DESIGN WITH TABLE	
			MessageUnit ='';
			MessageUnit += '<div class="MessageItem" id="'+NewMES[n].MessageID+'" id2="'+NewMES[n].MessageIdPat+'" id3="'+NewMES[n].MessageIdSender+'" style="float:left; width:100%; margin-top:10px; ">'; 
				MessageUnit += '<table class="MessageItemTable" style="margin:0 auto; background-color:#fdfdfd; border:1px solid #cacaca; border-radius:10px; border-collapse: separate !important;">'; 
					MessageUnit += '<tr class="MsgRow" style="border-radius:10px; border-collapse: separate !important;">'; 
						MessageUnit += '<td class="LeftZone" style="width:60px; text-align:center; border-top-left-radius:5px; border-bottom-left-radius:5px; text-align:center;">'; 
							var Picfound = 0;
									if (Picfound == 1) 
										{
										MessageUnit += '<img src="temp/'+queMED+'/PackagesTH_Encrypted/'+thumbnail+'" style="border:none; margin-left:0px; width: 40px; height: 40px; border:#cacaca; margin-top:13px;" class="img-circle">';
										}
									else
									{
										MessageUnit += '<div class="LetterCircleON" style="float:none; margin:0 auto; border:none; width:40px; height:40px; font-size:12px; background-color:' + whatcolor + ';"><p style="margin:0px; padding:0px; margin-top:13px;">'+ NewMES[n].MessageINIT +'</p></div>	';
									}	
								//MessageUnit += '</div>'; 
						MessageUnit += '</td>'; 
						MessageUnit += '<td class="CenterZone" style="width:380px;">'; 
									MessageUnit += '<div style="float:left; width:390px; padding-right:5px; padding-left:5px; '+newIndicator2+' margin-left:10px; <!--border-bottom:thin dotted #cacaca;-->"> '; 
										MessageUnit += '<div style="float:left; width:100%;  margin-top:10px; font-weight:bold;font-size:14px; color:#22aeff;">' +  NewMES[n].MessageNameSender + '</div><div style="float:left; width:100%; color:#54bc00; margin-top:-5px; font-style:italic;"><icon class="icon-caret-right"></icon>  '+NewMES[n].MessageNamePat+'</div><div style="color:#cacaca; float:right; margin-top:-30px; font-size:12px; font-weight:normal;"><span>    ' + formatDate + '</span> ' + '</div>' ;
										//MessageUnit += newIndicator;
										MessageUnit += '<p style="color:grey; font-weight:bold; font-size:14px; margin-bottom:0px; margin-top:0px;">' + NewMES[n].MessageSUBJ + '</p>';
										MessageUnit += '<p style="color:grey; margin-top:5px; margin-bottom:0px; font-size:14px; line-height:120%; font-weight:'+boldContent+'; ">' + NewMES[n].MessageCONT.replace(/sp0e/gi," ") + '</p>';
										
										var splitted = NewMES[n].MessageRIDS.split(" ");
										NumReports = splitted.length - 1;
										if (NumReports>0)
										{
											MessageUnit += '<div style="margin:0 auto; margin-top:10px; width:95%; <!--border:solid #cacaca; border-radius:5px;--> height:80px;">'; 
												...
												while (rn < NumReports)
												{
													...
													MessageUnit += '<img class="ThumbTimeline" id="'+splitted[rn]+'" style="height:70px; margin:5px; border:solid 1px #cacaca;" src="temp/'+queMED+'/PackagesTH_Encrypted/'+thumbnail+'">';
													rn++;
												}
											MessageUnit += '</div>'; 
										}	
										
										MessageUnit += '<p style="color:#cacaca; font-size:10px; margin-left:50px; margin-top:5px;"> <span style="margin-right:15px;"><i style="font-size:10px; margin-right:5px;" class="icon-mail-reply"></i>Reply</span><span style="margin-right:15px;"><i style="font-size:10px; margin-right:5px;" class="icon-arrow-right"></i>Forward</span><span style="margin-right:15px;"><i style="font-size:10px; margin-right:5px;" class="icon-bookmark"></i>Mark</span></p>';
									MessageUnit += '</div>'; 
						MessageUnit += '</td>'; 
						MessageUnit += '<td class="RightZone" id="'+NewMES[n].MessageID+'" id2="'+NewMES[n].MessageIdPat+'" id3="'+NewMES[n].MessageIdSender+'" style="width:60px; background-color:'+newIndicator3+'; border-top-right-radius:5px; border-bottom-right-radius:5px; text-align:center;">'; 
									
									var cadena = 'DecryptPatientPic.php?UserId='+NewMES[n].MessageIdPat;
									var RecTipo = LanzaAjax (cadena);
									//alert (cadena+'    Resultado: '+RecTipo);
									console.log(RecTipo);
									if (RecTipo != '/PatientImage/defaultDP.jpg') 
										{
										MessageUnit += '<img src="temp/'+queMED+'/'+NewMES[n].MessageIdPat+'.jpg" style="border:none; margin:0 auto;  width: 40px; height: 40px; border:#cacaca; " class="img-circle">';
										}
									else
									{
										MessageUnit += '<div class="LetterCircleON" style="float:none; margin:0 auto; border:none; width:40px; height:40px; font-size:12px; background-color:' + whatcolor + ';"><p style="margin:0px; padding:0px; margin-top:13px;">'+ NewMES[n].MessageInitialsPat +'</p></div>	';
									}	
						MessageUnit += '</td>'; 
					MessageUnit += '</tr>'; 
				MessageUnit += '</table>'; 
			...
			MesTimeline = MesTimeline + MessageUnit;
			n++;
		}
...
//checkImage('http://www.google.com/images/logdo_sm.gif');

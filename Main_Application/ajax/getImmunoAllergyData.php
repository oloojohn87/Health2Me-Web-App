...
echo '<style>';
echo 'div.timecell{';
echo '  background-color:#22aeff; margin:0 auto; text-align:center; color:white; font-size:12px;';
echo '  } ';
echo 'div.cellvacc{';
...
echo '				<div id="ImmunoInnerL" style="width:80%; height:100%; border:0px solid;">';
...
while ($m <= ($rowCounterAllergy-1) && isset($_GET['allergy']))
	{
	$iconR = "";
		if ($intensity[$pointerEntry[$m]]>3) $iconR = '-webkit-animation: glow 2s linear infinite;';
		$titleAl = '';
		if ($intensity[$pointerEntry[$m]] == 4) $titleAl = 'Very Strong Intensity Allergy'; 
		if ($intensity[$pointerEntry[$m]] == 5) $titleAl = 'EXTREME Intensity Allergy'; 
		$maxlight = 50;
		$minlight = 20;

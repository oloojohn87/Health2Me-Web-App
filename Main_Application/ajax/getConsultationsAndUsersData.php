    ...
    
{
   ...
   */
        for($i=0;$i<=23;$i++)
        {
            ...
            $query = $con->prepare("select consultationId from consults where DateTime between ? and ?".$scope_str);
            $query->bindValue(1, $curdate." ".$time.":00:00", PDO::PARAM_STR);
            $query->bindValue(2, $curdate." ".$time.":59:59", PDO::PARAM_STR);
            if($scope != 'H2M')
            ...
            $query1 = $con->prepare("select distinct(patient) from consults where DateTime between ? and ?".$scope_str);
            $query1->bindValue(1, $curdate." ".$time.":00:00", PDO::PARAM_STR);
            $query1->bindValue(2, $curdate." ".$time.":59:59", PDO::PARAM_STR);
            if($scope != 'H2M')
        ...
        $query2 = $con->prepare("SELECT DISTINCT Patient FROM consults WHERE DateTime LIKE '".date("Y-m-d")."%'".$scope_str);
        if($scope != 'H2M')
    ...
    elseif($period == 2)
    {
        ...
        $query2 = $con->prepare("SELECT DISTINCT Patient FROM consults WHERE DateTime BETWEEN ? AND ?".$scope_str);
        $query2->bindValue(1, $dateOfLastSunday, PDO::PARAM_STR);
        $query2->bindValue(2, $todayDate, PDO::PARAM_STR);
        if($scope != 'H2M')
                $query2->bindValue(1, $scope, PDO::PARAM_STR);
    ...
    elseif($period == 4)
    {
        ...
        while(strcmp($current_date, $todayDate) != 0)
        {
            ...
            $query = $con->prepare("select consultationId from consults where DateTime like ?".$scope_str);
            $query->bindValue(1, $compare_str."%", PDO::PARAM_STR);
            if($scope != 'H2M')
            ...
            $query1 = $con->prepare("select distinct(patient) from consults where DateTime like ?".$scope_str);
            $query1->bindValue(1, $compare_str."%", PDO::PARAM_STR);
            if($scope != 'H2M')

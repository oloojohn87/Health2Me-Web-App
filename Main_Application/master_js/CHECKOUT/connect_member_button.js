		...
		$.post("get_probe_protocols.php", {doctor: $("#MEDID").val()}, function(data, status)
		{
			...
			$.post("get_probe.php", {doctor: $("#MEDID").val(), patient: connectMemberSelected}, function(data, status)
			{
				...
				else
				{
					...
					$("#probe_protocols").data('prev', 0);
					$("#probe_time5").val(0);
					$("#probe_timezone").val(0);
					$("#probe_interval").val(0);
					$("#probe_method").val(0);
                    $("#edit_probes_button").attr('disabled', 'disabled');
				}
			});
...
function stripeBuyTokensResponseHandler(status, response) 
{
	...
	{
		$.post("purchaseCredits.php", {doctor: $("#MEDID").val(), quantity: $("#connectMemberNumCredits").val(), token: response.id}).done(function(data, status)
		{

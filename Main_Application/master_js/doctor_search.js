...
$(".my_doctors_delete_button").live('click', function()
{
    ...
    if (r == true) 
    {
        $.post("../../../ajax/edit_my_doctor.php", {delete_id: id}, function(data, status)
        {
...
$(".my_doctors_edit_button").live('click', function()
{
    ...
    $("#my_doctors_phone").val("");
    $("#my_doctors_email").val("");
    $.post("../../../ajax/get_my_doctors.php", {iddoc: id}, function(data, status)
    {
...
{
    var id = $(this).attr("id").split("_")[1];
    $.post("../../../ajax/invite_new_doctor.php", {doc_id: id, pat_id: $("#USERID").val()}, function(data, status)
    {
...
{
    $.post("../../../ajax/get_my_doctors.php", {idpac: $('#USERID').val()}, function(data, status)
    {
        var d = JSON.parse(data);
        var html = '';
        if(d.length > 0)
        {
            for(var i = 0; i < d.length; i++)
            {
                ...
                html += '<img class="doctor_pic" src="../../../ajax/identicon.php?size=25&hash=\''+d[i].hash+'\'" />';
...
$(".doctor_action_button").live('click', function()
{
    ...
    if($(this).text() != 'Select Doctor')
    {
        ...
        $.post("../../../ajax/getDoctorAvailableTimeranges.php", {id: $(this).parent().parent().children('input').eq(0).val()}, function(data, status)
        {
    ...
    else
    {
        ...
        $.post("../../../ajax/set_personal_doctor.php", {doctor_id: $(this).parent().parent().children('input').eq(0).val(), user_id: $("#USERID").val()}, function(data, status)
        {
            get_personal_doctor();
            $("#doctor_directory").css("display", "none");
            $("#personal_doctor").css("display", "block");
    ...
    $.post("../../../ajax/getTelemedDoctors.php", {grant_access: grant_access, starting_point: doctors_next_result, order: order_column, asc: order_direction, timezone: get_timezone_offset(), online_only: online_only, telemed_only: telemed_only, search_term: $("#search_bar").val(), speciality: sp, location: loc, personal_doctor_search: personal_doctor_search, pat_id: $("#USERID").val(), tag: doctor_search_tag}, function(data,status)
    {
        ...
        if(info['tag'] == doctor_search_tag)
        {
            ...
            for(var i = 0; i < docs.length; i++)
            {
                ...
                {
                    html += '<img class="doctor_pic" src="../../../ajax/identicon.php?size=25&hash='+docs[i]['image']+'" />';
                }
                ...
                var queUrl = '../../../ajax/scheduleCheck.php?id='+id_holder;
                $.ajax(
                {
                   url: queUrl,
                   dataType: "json",

...
function h2m_notification_action(id, type, receiver, sender, is_receiver_doctor, is_sender_doctor, auxilary, scope)
{
    ...
    {
        window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+auxilary;
    }
    else if(type == 'REPUPL' || type == 'SNDREQ')
    {
        if(is_receiver_doctor == 1)
        {
            if(is_sender_doctor == 1) window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+auxilary;
            else window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+sender;
        }
        else
        {
            window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+receiver;
        }
    }
    else if(type == 'SUMEDT')
    {
        if(is_receiver_doctor == 1)
        {
            window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+sender+'&OPENMODAL=1';
        }
        else
        {
            window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+receiver+'&OPENMODAL=1';
        }
    }
    else if(type == 'MEDALR')
    {
        if(is_receiver_doctor == 1)
        {
            window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+sender+'&OPENMEDMODAL=1';
        }
        else
        {
            window.location.href = '../../patientdetails/html/PatientDetails.php?IdUsu='+receiver+'&OPENMEDMODAL=1';
        }
    }
    else if(type == 'NEWMES')
    {
        if(is_receiver_doctor == 1)
        {
            if(is_sender_doctor == 1)
                window.location.href = '../../common/Messages.php?isDoctors=1&id=' + auxilary;
            else
                window.location.href = '../../common/Messages.php?isDoctors=0&id=' + auxilary;
        }
        else
        {
    ...
    {
        window.location.href = '../../membernetwork/html/MemberNetworkCATA.php?LOADPATIENT='+sender;
    }
    else if (type == 'NEWAPP' && scope == 'CATAPULT')
    {
        if(is_receiver_doctor == 1)
            ...
            window.location.href = '../../../../catapult_telemedicine/catapult_telemed_pat.php?doc_id=' + sender + '&pat_id=' + receiver;
	}
...
$.fn.h2m_notifications = function(dictionary) 
{
    ...
    $.post('../../../ajax/load_notifications.php', {user: user, type: type}, function(data, status)
    {
        o.empty();
        o.html('<style> \
        ...
        for(var i = 0; i < res.length; i++)
        {
            ...
            if(res[i].auxilary != null) {
                ...
                //CHECK THE APPUPD TIME AND THE CURRENT SYSTEM TIME TO DECIDE WHETHER TO DELETE THE NOTIFICATION
                if(res[i].type == 'APPUPD') {
                    ...
                    if (auxDate <= d) {
                        $.post("../../../ajax/dismissNotification.php", {id: res[i].id}, function() {});
                    }
                }
            }            
...
$(".notification_row_info").live('click', function()
{
    ...
    {
        //$.post("../../ajax/dismissNotification.php", {id: id}, function(data, status)
        //{
...
{
    $.get('../../master_js/jquery-lang-js-master/js/langpack/'+initial_language+'.json', function(data, status)
    {
        var json = data;

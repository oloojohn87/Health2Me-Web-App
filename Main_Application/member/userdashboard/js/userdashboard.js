...
$("#verify_cc").click(function() {
	...
	$.post("../../../ajax/piggy_bank.php", {user: mem_id}, function(data, status)
	{
		//console.log(data);
...
$("#check_cc_amounts").click(function() {
	...
	//alert(amount2);
	$.post("../../../ajax/verify_charges.php", {user: mem_id, charge1: amount1, charge2: amount2}, function(data, status)
	{
		//console.log(data);
...
function fileSelected() {
    ...
    if (file) {
        ...
        if(profile_pic.indexOf('defaultDP') > -1) profile_pic = '../../../ajax/PatientImage/'+mem_id+'.jpg';
        ...
        setTimeout(function() {document.getElementById('profile_pic').src = '../../../ajax/PatientImage/'+mem_id+'.jpg';}, 500);
...
function uploadFile2() {
    ...
    xhr.open("POST", "../../../ajax/uploadifyPatient.php?pullfile="+mem_id+"&upload="+upload_type);
    xhr.send(fd);
}
...
function uploadComplete(evt) {
    ...
    $.post("../../../ajax/compare_images.php", {user: mem_id}, function(data, status)
	{
		//console.log(data);
...
//ticker code
  ...
  var gritterOptions = {
    ...
    $(document).ready(function() {
        ...
        // notifications
        function displaynotification(status,message){
            getScript('../../../ajax/realtime-notifications/lib/gritter/js/jquery.gritter.min.js');
            var gritterOptions = {
                                    title: status,
                                    text: message,
                                    image:'images/Icono_H2M.png',
        ...
        { 
            ...
            $("#main_notif_container").h2m_notifications({type: 'PAT', patient: $('#USERID').val(), scope: 'CATAPULT'});
        ...
        $.get("../../../ajax/createSendStream.php?ElementDOM=na&EntryTypegroup=0&Index=1&Usuario="+mem_id, function(data, status)
        {
        ...
        $(".user_button").live('click', function()
        {
            ...
            {   
                ...
                $.post("../../../ajax/logout.php", {logging_out_family_member: true}, function(data, status)
                {
                    $.post("../../../ajax/loginUSER.php", {Nombre: email},  function(data, status)
                    {
                        location.reload();
                    });
                });
        ...
        $(".change_user_dropdown_button").live('click', function(e)
        {
            ...
            {
                $.post("../../../ajax/logout.php", {logging_out_family_member: true}, function()
                {
                    $.post("../../../ajax/loginUSER.php", {Nombre: email},  function(data, status)
                    {
                        location.reload();
        ...
        $("#upgrade_premium_button").on('click', function()
        {
            ...
            $.post("../../../ajax/change_subscription.php", {user: $("#USERID").val(), plan: 1}, function(data, status)
            {
        ...
        {
            $("#upgrade_loading_bar").css('visibility', 'visible');
            $.post("../../../ajax/change_subscription.php", {user: $("#USERID").val(), plan: 2}, function(data, status)
            {
                $("#upgrade_loading_bar").css('visibility', 'hidden');
                if(data == 'GOOD')
                {
        ...
        $("#grant_admin_access_button").on('click', function()
        {
            ...
            $.post("../../../ajax/EditUserFam.php", {User: $("#USERID").val(), Access: access, Grant_Access: true});
        });
        ...
        $('button[id^="family_member_delete_"]').live('click', function()
        {
            ...
            if (r == true) 
            {
                $.post("../../../ajax/EditUserFam.php", {Delete: true, User: id}, function(data, status)
                {
                    if(data == '1')
                    {
        ...
        $('button[id^="family_member_edit_"]').live('click', function()
        {
           // stausOfFamilyEditingMode = 2;
            ...
            family_editing_mode = 2;
            $.post("../../../ajax/EditUserFam.php", {Get_info: true, User: id}, function(data, status)
            {
                //console.log(data);
        ...
        $("#telemed_deny_button").live("click", function()
        {
            ...
            $.post("../../../ajax/reset_appointment.php", {pat_id: $("#USERID").val()}, function(){});
        });
...
//		$.post("GetConnectedLight.php", {User: queMED, NReports: 3, Group: group}, function(data, status)
		var queUrl = '../../../ajax/GetSummaryDataFull.php?User='+UserID;
		...
		var queUrlPartial = '../../../ajax/GetSummaryData.php?User='+UserID;
 		//console.log('url:  '+queUrl);
        $.ajax(
		{
			url: queUrlPartial,
	  ...
	  while (n < 7)
	  {
 	  ...
 	  // Get Basic Icons and Colors for every type of report
 	  	var RepData = Array();
		// Ajax call to retrieve a JSON Array **php return array** 
		var queUrl = '../../../ajax/GetReportSet.php';
		$.ajax(
		{
			url: queUrl,
			dataType: "json",
 	  ...
 	  // Get Report Data for this user
		...
		// Ajax call to retrieve a JSON Array **php return array** 
		var queUrl = '../../../ajax/GetReportNumbers.php?User='+UserID;
        //console.log('url = '+queUrl);
		$.ajax(
		{
			url: queUrl,
	...
	if (Canvas1State == 0)	{
       ...
       //Start of new code added by Pallab
		...
		translation3 = 'on';
			...
			namedoctor = LanzaAjax ('../../../ajax/getDoctorName.php?IdDoctor='+doctorId); //Changed this statement from getDoctorName.php?IdDoctor='+bestVerified to getDoctorName.php?IdDoctor='+doctorId by Pallab
...
//		rgb(74, 134, 54);
	...
	$('#myCanvas').live('click',function(){
        ...
        $("#summary_modal").html('<iframe src="../../../ajax/medicalPassport.php?modal=1&IdUsu='+myClass+'" scrolling="no" style="width:1000px;height:690px; margin: 0px; border: 0px solid #FFF; outline: 0px; padding: 0px; overflow: hidden;"></iframe>');
    ...
    $('#editPatient').live('click',function(){
		...
		window.location.replace('../../patientdetails/html/PatientDetails.php?IdUsu='+userID); // Changed the url value from patientdetailMED-newUSER.php to patientdetailMED-new.php
    });    
	...
	$('#ButtonReview').live('click',function(){
        ...
        $("#summary_modal").dialog({bigframe: true, width: 1050, height: 690, resize: false, modal: true}).onclose(function(data,success)
           {
             $.post("../../../ajax/SavePatientSummaryAsPDF.php",{IdUsu: mem_id},function(data,success)
               {
              });
...
//	$('#UpdateSumm').live('click',function(){
	...
	$('#myCanvas2').live('click',function(){
		var myClass = $('#USERID').val();
		window.location.replace('../../patientdetails/html/PatientDetails.php?IdUsu='+myClass); // Changed the url value from patientdetailMED-newUSER.php to patientdetailMED-new.php
	});
	...
	$('#newinbox').live('click',function(){
       //alert('trigger');
	   var queUrl ='../../../ajax/getInboxMessageUSER.php?IdMED='+both_id+'&patient='+mem_id;
	   //alert (queUrl);
    ...
    $('#newoutbox').live('click',function(){
      //alert('trigger');
	   var queUrl ='../../../ajax/getOutboxMessageUSER.php?IdMED='+both_id+'&patient='+mem_id;
    ...
    function load_credit_cards(cards)
    {
        ...
        $('button[id^="clear-credit-card"]').on('click', function()
        {
            ...
            $("#setup_modal_notification").css("background-color", "#FFF");
            $("#setup_modal_notification_container").css("opacity", "1.0");
            $.post("../../../ajax/change_credit_card.php", {type: '2', action: '2', id: $("#USERID").val(), card_id: card_id}, function(data, status)
            {
                //console.log(data);
                $("#setup_modal_notification_container").css("opacity", "0.0");
                if(data == '1')
                {
                    ...
                    $.post("../../../ajax/get_user_info.php", {id: $("#USERID").val()}, function(data, status)
                    {
                        var info = JSON.parse(data);
    ...
    function stripeResponseHandler(status, response) {
        ...
        else 
        {
            ...
            $("#setup_modal_notification").css("background-color", "#FFF");
            $("#setup_modal_notification_container").css("opacity", "1.0");
            $.post("../../../ajax/change_credit_card.php", {type: '2', action: '1', id: $("#USERID").val(), token: response.id}, function(data, status)
            {
                //console.log(data);
                $("#setup_modal_notification_container").css("opacity", "0.0");
                if(data == '1')
    ...
    $('#timezone_button').on('click', function()
    {
        ...
        $.post("../../../ajax/update_user.php", {id: $("#USERID").val(), timezone: $("#timezone_picker").val().toString()}, function(data, status)
        {
    ...
    {
        $("#update_spinning_icon").css("visibility", "visible");
        $.post("../../../ajax/update_user.php", {id: $("#USERID").val(), location: $("#country_setup").val(), location2: $("#state_setup").val()}, function(data, status)
        {
            $("#update_spinning_icon").css("visibility", "hidden");
            $("#setup_modal_notification").css("background-color", "#52D859");
    ...
    {
        $("#update_spinning_icon").css("visibility", "visible");
        $.post("../../../ajax/update_user.php", {id: $("#USERID").val(), email: $("#setup_email").val(), phone: $("#setup_phone").val()}, function(data, status)
        {
            $("#update_spinning_icon").css("visibility", "hidden");
            //console.log(data);
            if(data.length > 0)
    ...
    {
        $.post("../../../ajax/validate_password.php", {pat_id: $("#USERID").val(), pw: $("#pw1").val()}, function(data, status)
        {
    ...
    $("#change_password_finish_button").live('click', function()
    {
        if($("#pw2").val() == $("#pw3").val())
        {
            ...
            $.post("../../../ajax/update_patient.php", {new_pw: $("#pw2").val(), pat_id: $("#USERID").val()}, function(data, status)
            {
                $("#pw2").val("");
                $("#pw3").val("");
                $("#setup_modal_notification").css("background-color", "#52D859");
    ...
    $("#find_doctor_next_button").live('click', function()
    {
        ...
        else if(find_doctor_page == 31 || find_doctor_page == 21 || find_doctor_page == 11)
        {
            ...
            if(find_doctor_page == 31 || find_doctor_page == 11)
            {
                ...
                if($("#speciality").val() != null)
                {
                    ...
                    $.post("../../../ajax/find_doctor.php", {type: $("#speciality").val(), location_1: loc_1, location_2: loc_2, must_be_available: mba}, function(data, status)
                    {
                        //console.log(data);
                        if(data != 'none')
                        {
                            ...
                            $.post("../../../ajax/getDoctorAvailableTimeranges.php", {id: info['id']}, function(data2, status)
                            {
                                //console.log(data2);
        ...
        else if(find_doctor_page == 33 || find_doctor_page == 23 || find_doctor_page == 26 || find_doctor_page == 12)
        {
            if(find_doctor_page == 33)
            {
                ...
                $.post("../../../ajax/add_appointment.php", {medid: info[1], patid: $("#USERID").val(), date: date_selected, start_time: getSlotStartTime(time_selected), end_time: getSlotEndTime(time_selected), video: type, timezone: selected_timezone, type: 'pat'}, function(data,status)
                {
                    if(data != '-1')
                    {
                        $.get("../../../ajax/send_appointment_email.php?id="+data+"&type=patient", function(data, status)
                        {
                        });
                    }
                });
            ...
            if(find_doctor_page == 26 || find_doctor_page == 12)
            {
                ...
                if(consultation_type == 1)
                {
                    ...
                    $.post("../../../ajax/start_telemed_videocall.php", {pat_phone: $("#USERPHONE").val(), doc_phone: info[2], doc_id: info[1], pat_id: $("#USERID").val(), doc_name: (info[3] + ' ' + info[4]), pat_name: ($("#USERNAME").val() + ' ' + $("#USERSURNAME").val())}, function(data, status)
                    {
                            ...
                            if(data == 1)
                            {
                                ...
                                }, function(data, status)
                                {
                                    ...
                                    webrtc.on('readyToCall', function () 
                                    {
                                        //video_recorder = RecordRTC(webrtc.webrtc.localStream, video_record_options);
                                        //audio_recorder = RecordRTC(webrtc.webrtc.localStream, audio_record_options);
                                        console.log('JOINING ROOM: ' + 'health2me_room_' + info[1] + '_' + $("#USERID").val());
                                        webrtc.joinRoom('health2me_room_' + info[1] + '_' + $("#USERID").val());
                                        //call();
                                        //$.post("update_webrtc.php", {doc_id: doctor_id, status: 1}, function(data, status){console.log(data);});
                                        //setInterval(function(){$.post("update_webrtc.php", {doc_id: doctor_id, update_lastseen: true}, function(data, status){console.log(data);});}, 30000);
                                        //$(window).unbind("beforeunload");
                ...
                {
                    $("#video_telemedicine_loading_bar").css("visibility", "visible");
                    $.post("../../../ajax/start_telemed_phonecall.php?stage=init-call", {grant_access: 'H2M', pat_phone: $("#USERPHONE").val(), doc_phone: info[2], doc_id: info[1], pat_id: $("#USERID").val(), doc_name: (info[3] + ' ' + info[4]), pat_name: ($("#USERNAME").val() + ' ' + $("#USERSURNAME").val()), override: false, message_text: '', language:'en'}, function(data, status)
                    {
                        if(data.substr(0, 2) == 'IC')
                        {
                            $("#find_doctor_modal").dialog('close');
    ...
    $("[id^='recdoc_']").live('click', function()
    {
        ...
        $.post("../../../ajax/getDoctorAvailableTimeranges.php", {id: info[1]}, function(data, status)
        {
            var info = JSON.parse(data);
            for(var i = 0; i < 7; i++)
            {
    ...
    $("#find_doctor_general_practicioner").live('click', function()
    {
        ...
        $.post("../../../ajax/find_doctor.php", {type: "General Practice", location_1: loc_1, location_2: loc_2, must_be_available: mba}, function(data, status)
        {
            ...
            if(data != 'none')
            {
                ...
                selected_doctor_info = "recdoc_"+info['id']+"_"+info['phone']+"_"+info['name']+"_"+info['location'];
                //console.log(selected_doctor_info);
                $.post("../../../ajax/getDoctorAvailableTimeranges.php", {id: info['id']}, function(data, status)
                {
                    //console.log(data);
                    var info = JSON.parse(data);
                    for(var i = 0; i < 7; i++)
    ...
    $('#find_doctor_button').live('click', function()
    {
        if(doctor_to_connect.length > 0 || type_of_doctor_to_find.length > 0)
        {
            ...
            $.post("../../../ajax/find_doctor.php", {type: type_of_doctor_to_find, id: info[0]}, function(data, status)
            {
                //console.log(data);
                if(data == 'none')
                {
    ...
    $('#phone_call_button').live('click',function(e)
    {
        ...
        $.post("../../../ajax/start_telemed_phonecall.php", {pat_phone: $("#USERPHONE").val(), doc_phone: info[1], doc_id: info[0], pat_id: $("#USERID").val(), doc_name: (info[2] + ' ' + info[3]), pat_name: ($("#USERNAME").val() + ' ' + $("#USERSURNAME").val())}, function(data, status)
        {
            //console.log(data);
        });
    });
    $('#video_call_button').live('click',function(e)
     {
        $("#find_doctor_modal").dialog("close");
         var info = doctor_to_connect.split("_");
        $.post("../../../ajax/start_telemed_videocall.php", {pat_phone: $("#USERPHONE").val(), doc_phone: info[1], doc_id: info[0], pat_id: $("#USERID").val(), doc_name: (info[2] + ' ' + info[3]), pat_name: ($("#USERNAME").val() + ' ' + $("#USERSURNAME").val())}, function(data, status)
        {
            //console.log(data);
            if(data == 1)
            {
    ...
    $('#sendmessages_inbox').live('click',function(){
         ...
         var cadena='../../../ajax/sendMessageUSER.php?sender='+IdDocOrigin+'&receiver='+IdDoctor+'&patient='+IdPaciente+''+'&content='+content+'&subject='+subject+'&attachments='+reportids+'&connection_id=0&tofrom=to';
         ...
         var cadena='../../../ajax/push_serverUSER.php?FromDoctorName='+UserName+'&FromDoctorSurname='+UserSurname+'&FromDoctorId='+IdDoctor+'&Patientname='+UserName+'&PatientSurname='+UserSurname+'&IdUsu='+IdPaciente+'&message= New Message <br>From: '+UserName+' '+UserSurname+' <br>Subject: '+(subject).replace(/RE:/,'')+'&NotifType=1&channel='+IdDoctor+'&Selector=2';
         //alert(cadena);
         var RecTipo=LanzaAjax(cadena);
    ...
    $('.CFILA').live('click',function(){
       ...
       var cadena='../../../ajax/getDoctorMessage.php?msgid='+id;
       var RecTipo=LanzaAjax(cadena);
	   ...
	   if(reportsattached){
               ...
               //alert ('Usuario: '+Usuario+' MedID: '+MedID);
                var queUrl ='../../../ajax/CreateAttachmentStream.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&Reports='+reportsattached;
	   ...
	   $('#attachreports').hide();
	   $('#message_modal').trigger('click');
	   var cadena='../../../ajax/setMessageStatusUSER.php?msgid='+id;
	   var RecTipo=LanzaAjax(cadena);
	   /*setTimeout(function(){
   ...
   $("#Reply").show();
    ...
    $("#BotonBusquedaPac").click(function(event) {
    	    ...
    	    */
			...
			// alert(UserInput);
    	    var queUrl ='../../../ajax/getFullUsersMED.php?Usuario='+UserInput+'&IdMed='+queMED+'&Usuario='+UserInput+'&Group='+onlyGroup;
			//alert(queUrl);
    ...
    $("#BotonBusquedaPacCOMP").click(function(event) {
    	    ...
    	    var serviceURL = '../../../ajax/getpines.php' + '?Usuario=' + UserInput; 
    	    getLifePines(serviceURL);
    	    var longit = Object.keys(pines).length;	
    	    //alert (longit);
    	    var queUrl ='../../../ajax/getFullUsers.php?Usuario='+UserInput+'&NReports='+longit;
	...
	$('#ButtonWeemo').live('click',function(){
		$.ajax({
			 url: '../../../ajax/weemo_test.php?calleeID='+doc_email,
			 method: 'get',
				success: function(data){
				var htmlContent = data;
				var e = document.createElement('div');
...
*/
 	...
 	$('#datatable_1').dataTable( {
		...
		"bDestroy": true,
		"bRetrieve": true,
		"sAjaxSource": "../../../ajax/getBLOCKS.php"
	} );
...
//Start of code inserted from userdashboard-new-pallab.php
    ...
    $("#CaptureEmail2Send2Doc").on('click',function(){
        ...
        $.get("../../../ajax/ValidateSendEmailDoc.php?emailId="+emailId+"&user="+user+"&idpins="+idpins,function(data,status)
              {
 			    var UserID = mem_id;  		
                var queUrl = '../../../ajax/GetDoctorId.php?doctoremail='+emailId;
                ...
                */
                ...
                $.post("../../../ajax/add_notification.php", {patient_id: UserID, doctor_id: doctorid, reports: idpins}, function(data, status)
                {
...
$("#request-button").on('click',function(){
	...
	if ($emailId == ""){
	 ...
	 $.post("../../../ajax/medical_release.php", {patient_name:$patient_name,ssnum:$patient_ssn, dob:$patient_dob,telephone:$patient_phone,output:$sig_img,doc_email:$emailId,recordtype:$record_type}, function(data, status)	{
		//console.log(data);
		//DEBUG: alert(data);
    ...
    var success = 0;    
     $.get("../../../ajax/RequestReportsFromExternalDoc.php?emailId="+emailId+"&user="+user+"&message="+messageForDoc,function(data,status)
              {		

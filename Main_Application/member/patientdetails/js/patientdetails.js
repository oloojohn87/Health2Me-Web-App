...
function checkNPAvailability(){
        ...
        $.post("../../../ajax/checkNPAvailability.php", {id: get_idusu, date: np_date, time: np_time}, function(data, status){
...
function showTempPassword(){
	$.post('../../../ajax/getTempPassword.php', {id:mem_id})
	.done(function(data){
...
function setLocation(id) {
    var locationURL = '../../../ajax/setLocation.php?id='+id;
    $.ajax(
        {
            url: locationURL,
            dataType: "json",
    ...
    if(cname2 == ''){
        ...
        $("#ePrescribe_Modal").html('<iframe src="../../../modules/mdtoolbox/index.php?modal=1&IdUsu='+myClass+'&medid='+myDoc+'&cname='+cname2+'&address1='+address12+'&address2='+address22+'&city='+city2+'&state='+state2+'&zip='+zip2+'&phone='+phone2+'&fax='+fax2+'&id='+id2+'&practice='+practice_holder+'&practiceid='+practiceid_holder+'" width="1000" height="660" scrolling="no" style="width:1000px;height:660px; margin: 0px; border: 0px solid #FFF; outline: 0px; padding: 0px; overflow: hidden;"></iframe>');
...
function setPractice(id) {
    var locationURL = '../../../ajax/setLocation.php?id='+id+'&show=practice';
    $.ajax(
        {
            url: locationURL,
            dataType: "json",
...
function pullLocationsList(id) {
    var myDoc = $('#MEDID').val();
    var DirURL = '../../../ajax/getLocationsList.php?docid='+myDoc+'&id='+id;
    $.ajax(
        {
            url: DirURL,
            dataType: "html",
...
function uploadFile2() {
    ...
    xhr.open("POST", "../../../ajax/uploadifyPatient.php?"+uploader, true);
...
function inform_about_session()
{
    if(both_id != ''){
        $.ajax({
            url: '../../../ajax/ongoing_sessions.php?userid='+both_id,
            success: function(data){
                //alert('done');
            }
        });
...
function ShowTimeOutWarning()
{
    ...
    var a=0;
    window.location = '../../../ajax/timeout.php';
}
...
$(document).ready(function() {
    ...
    function share_reports ()
    {
        ...
        var MedID = $("#MEDID").val();
        setTimeout(function(){
            var queUrl ="../../../ajax/createAttachmentStreamNEWTEST.php?ElementDOM=na&EntryTypegroup="+EntryTypegroup+"&Usuario="+Usuario+"&MedID="+MedID+"&display_member=yes";
    ...
    function grabTimeline(id){
        ...
        {
            $.post("../../../ajax/get_my_personal_doctors.php", {id: $("#USERID").val()}, function(data, status)
            {
    ...
    $("#TrackingButton").on('click', function()
    {
        ...
        $.post("../../../ajax/get_probe.php", {doctor: dctr, patient: $("#USERID").val()}, function(data, status)
        {
            var d0 = JSON.parse(data);
            var probe = d0['probeID'];
            tracking_probe_id = probe;
            
            $.post("../../../ajax/get_probe_protocols.php", {id: d0['protocolID']}, function(data, status)
            {
                ...
                $.post('../../../ajax/get_probe_graph_data.php', {probeID: probe, question: question}, function(data, status)
                {
    ...
    function load_medication_reminders()
    {
        ...
        $.post("../../../ajax/get_medication_reminders.php", {user: $("#USERID").val()}, function(data, status)
        {
            console.log(data);
            var d = JSON.parse(data);
            if(d.length > 0)
    ...
    function update_medication_reminder()
    {
        ...
        $.post("../../../ajax/update_medication_reminders.php", {start: date, timezone: $("#reminder_timezone").val(), alert: $("#reminder_alert").val(), frequency: $("#reminder_frequency").val(), unit: $("#reminder_frequency_unit").val(), on: on, id: medication_reminder_selected, edit: 1}, function(data, status)
        {
            console.log(data);
        });
    }
    ...
    $(".reminders_edit_button").live('click', function()
    {
        ...
        $.post("../../../ajax/get_medication_reminders.php", {user: $("#USERID").val(), id: id}, function(data, status)
        {
            var d = JSON.parse(data);
            console.log(data);
    ...
    $(".reminders_delete_button").live('click', function()
    {
        ...
        swal({title: "Delete Reminder",   
             ...
             {   
                if (isConfirm) 
                {
                    $.post("../../../ajax/update_medication_reminders.php", {id: id, delete: 1}, function(data, status)
                    {
    ...
    {
        $.post("../../../ajax/update_medication_reminders.php", {user_id: $("#USERID").val(), name: medication_selected, image: medication_selected_image}, function(data, status)
        {
    ...
    $("#shareFilesButton").on('click', function()
    {
        ...
        $("input[type=checkbox][id^=\"reportcol\"]").each(
            ...
            $.post( "../../../ajax/display_pin_for_member.php", { reports: reportids })
            .done(function( data ) {
    ...
    $("#probe_protocols").on('change', function()
    {
        ...
        swal({title: "Probe Change",   
             ...
             {   
                if (isConfirm) 
                {
                    $.post("../../../ajax/save_probe_alerts.php", {clear: true, doctor: $("#MEDID").val(), patient: selected_patient}, function(data, status)
                    {
                    });
                    save_prove();
    ...
    $("#probe_alert_button").on('click', function()
    {
        ...
        {
            $.post("../../../ajax/get_probe_protocols.php", {doctor: $("#MEDID").val()}, function(data, status)
            {
                var d = JSON.parse(data);
                ...
                $.post('../../../ajax/get_probe_alerts.php', {doctor: $("#MEDID").val(), patient: selected_patient}, function(data, status)
                {
                    console.log(data);
                    alerts = [{}, {}, {}, {}, {}];
                    var d = JSON.parse(data);
    ...
    $('#probe_alerts_save_button').live('click', function()
    {
		...
		if(first_value_parsed <= second_value_parsed){
			$.post("../../../ajax/save_probe_alerts.php", {alerts: JSON.stringify(alerts), doctor: $("#MEDID").val(), patient: selected_patient}, function(data, status)
			{
				$("#edit_probe_alerts").css("display", "none");
				$("#manage_user_probe").css("display", "block");
			});
    ...
    $("#save_probes_button").on('click', function()
    {
        ...
        {
            $.post("../../../ajax/toggle_probe.php", {doctor: $("#MEDID").val(), patient: selected_patient, status: 'null', protocol: $("#probe_protocols").val(), time: $("#probe_time5").val(), timezone: $("#probe_timezone").val(), interval: $("#probe_interval").val(), request: $("#probe_method").val(), save: 1}, function(data, status)
            {
    ...
    $("#edit_probes_button").on('click', function()
    {
        ...
        $.post("../../../ajax/get_probe_protocols.php", {id: probe_id}, function(data, status)
        {
            console.log(data);
            var d = JSON.parse(data);
    ...
    $("#launch_probes_button").on('click', function()
    {
        ...
        $.post('../../../ajax/launch_probe.php', {doc: $("#MEDID").val(), pat: selected_patient}, function(data, status)
        {
            console.log(data);
            if(data == '1')
            {
    ...
    $('#probe_delete_button').live('click', function()
    {
        ...
        if(probe_id > 0)
        {
            ...
            {   
                if (isConfirm) 
                {     
                    $.post("../../../ajax/delete_probe_protocol.php", {id: probe_id}, function(d, status)
                    {
    ...
    if($("#CHECKOUT").val() == 1)
    {
        ...
        $("#checkout_ask_yes_button").on("click", function()
        {
            ...
            console.log('../../../ajax/share_reports.php?Usuario='+connectMemberSelected);
            $("#share_files_container2").html('<iframe id="share_reports" src="../../../ajax/share_reports.php?Usuario='+connectMemberSelected+'" style="border: 0px solid #FFF; width: 100%; height: 490px;" />');
            ...
            $.post("../../../ajax/get_user_info.php", {id: selected_patient}, function(data, status)
            {
    ...
    var videoTelemed = $("#videoTelemed").dialog({bigframe: true, /*open: function(event, ui) { $(".ui-dialog-titlebar-close", ui.dialog).hide(); },*/ width: 550, height: 440, minWidth: 550, minHeight: 440, sticky: false, modal: false, autoOpen: false, dialogClass: 'telemedModalClass', position: {my: "left+70", of: "#NombreComp"}, resize: function( event, ui ) 
                                                  ...
                                                  {
                                                      $.post("../../../ajax/update_webrtc.php", {doc_id: $("#MEDID").val(), status: 0}, function(){});
                                                  }});
    ...
    if(telemedOn)
    {
        ...
        $.post("../../../ajax/retrieveLatestConsultation.php", {doctor: $("#MEDID").val(), patient: $("#USERID").val()}, function(data, status)
               {
...

        {
            ...
            }, function(data, status)
            {
                ...
                webrtc.on('readyToCall', function () 
                {
                    ...
                    $.post("../../../ajax/update_webrtc.php", {doc_id: $("#MEDID").val(), pat_id: $("#USERID").val(), status: 1, add_to_recent_doctors: 1}, function(){});
                    setInterval(function(){$.post("../../../ajax/update_webrtc.php", {doc_id: $("#MEDID").val(), update_lastseen: true}, function(data, status){console.log(data);});}, 30000);
                    $("#remoteVideo").children("video").each(function()
                                                             {
                        $(this).css("height", "100%");
                        $(this).css("width", "100%");
    ...
    $('.PrintHighlighted').live('click',function()	
        ...
        var link = '../../../ajax/mergePDF.php?referralID='+myClass;
        $.ajax(
            {
                url: link,
                dataType: "html",
    ...
    $('#AddEvolution').live('click',function()	
    {
        ...
        var cadena = '../../../ajax/add_evolutions.php?userid='+userid+'&date='+date+'&text='+text;
        
        var RecTipo = LanzaAjax (cadena);
        //alert(cadena);
        if(RecTipo=='success')
        {
            var cadena = '../../../ajax/EvolutionPDF.php?idusu='+userid;
            //alert(cadena);
            var RecTipo = LanzaAjax (cadena);
    ...
    $('#AddNote').live('click',function()	
        ...
        $.post("../../../ajax/storePatientNotesTemp.php",{doctorId: doctorId,findings: findings,recommendations: recommendations},function(data,success)
               {
            console.log(data);
        });
    ...
    $('#NewMES').live('click',function(){
        var queUrl ='../../../ajax/getInboxMessageUNREAD.php?IdMED='+med_id+'&patient='+mem_id+'&sendingdoc='+otherdocid[i];		
        ...
        while (n < Pa)
        {
            ...
            if (NumReports>0)
            {
                ...
                while (rn < NumReports)
                {
                    var cadena = '../../../ajax/DecryptFileId.php?reportid='+splitted[rn]+'&queMed='+med_id;
                    var RecTipo = LanzaAjax (cadena);
    ...
    function loadTimeline(){
		...
		*/
        ...
        var DirURL = '../../../ajax/TimelineNEW.php?usuario='+usuario+'&medid='+IdMed+'&isdoctor='+isDoctor;
console.log(usuario+IdMed+isDoctor);
    ...
    $(".icon-trash").live('click',function() {
        ...
        var cadena = '../../../ajax/getReportStatus.php?IdPin='+idpin;
        ...
        else if(packetstatus==3)
        {
            ...
            var cadena = '../../../ajax/deleteReports.php?IdPin='+idpin+'&state='+packetstatus;
            ...
            var MEDIO = 0;
            var cadena = '../../../ajax/LogEvent.php?IDPIN='+idpin+'&Content='+Content+'&VIEWIdUser='+VIEWIdUser+'&VIEWIdMed='+VIEWIdMed+'&MEDIO='+MEDIO;
            var RecTipo = LanzaAjax (cadena);
    ...
    $("#EvolutionToggle").live('click',function() {
        ...
        if(fileFound)
        {
            ...
            var cadena = '../../../ajax/DecryptFile.php?rawimage='+rawimage+'&queMed='+med_id;
            var RecTipo = LanzaAjax (cadena);
    ...
    $("#Summary_Button").on('click',function(e){
        ...
        var myClass = $('#USERID').val();
        //alert('reach here');
        $("#summary_modal").html('<iframe src="../../../ajax/medicalPassport.php?modal=1&IdUsu='+myClass+'" width="1000" height="660" scrolling="no" style="width:1000px;height:660px; margin: 0px; border: 0px solid #FFF; outline: 0px; padding: 0px; overflow: hidden;"></iframe>');
    ...
    function pullPracticesList() {
        var myDoc = $('#MEDID').val();
        var DirURL = '../../../ajax/getLocationsList.php?docid='+myDoc+'&show=practice';
        $.ajax(
            {
                url: DirURL,
                dataType: "html",
    ...
    $("#create_location").on('click', function()
        ...
        if(clinic_name == ''){
            ...
            var url = '../../../ajax/createLocationsData.php?cname='+clinic_name+'&pid='+practice_id+'&address1='+address1_holder+'&address2='+address2_holder+'&city='+city_holder+'&state='+state_holder+'&zip='+zip_holder+'&phone='+phone_holder+'&fax='+fax_holder+'&docid='+myDoc;
            //console.log(url);
            var Rectipo = LanzaAjax(url);
    ...
    $("#create_practice").on('click', function()
        ...
        if(practice_name == ''){
            ...
            var url = '../../../ajax/createLocationsData.php?practice='+practice_name+'&docid='+myDoc;
            //console.log(url);
            var Rectipo = LanzaAjax(url);
    ...
    $("#create_credentials").on('click', function()
        ...
        if(npi_number == '' | npi_number.length < 10){
            ...
            var url = '../../../ajax/createCredentials.php?docid='+myDoc+'&npi='+npi_number+'&dea='+dea_number;
            //console.log(url);
            var Rectipo = LanzaAjax(url);
    ...
    $(".Telemed_Close_Button").on('click',function(e)
        ...
        $.get("../../../ajax/getNotes.php?doctorId="+doctorId,function(data,status)
              {
            console.log(data);
            ...
            $.post("../../../ajax/addPatientNotes.php",{doctorId: doctorId,userId: userId,findings:findings,recommendations: recommendations},function(data,success)
                   {


                $.post("../../../ajax/SavePatientSummaryAsPDF_table.php",{IdUsu:userId},function(data,status)
                       {
						   console.log(userId+doctorId)
                    console.log("Result Data for Summary: "+data);
                    $.post("../../../ajax/charge_for_consultation.php", {user: userId, doctor: doctorId}, function(data, status)
                           {
                        cost = data;
...
console.log(data);
                        ...
                        if(telemedValue != 2)
                        {
                            $.post("../../../ajax/video_appointment_callback.php", {doc_id: doctorId, pat_id: userId}, function(data, status)
                                   {
                                $.get("../../../ajax/sendPatientNotesAndSummary.php?doctorId="+doctorId+"&userId="+userId+"&cost="+cost+"&lang="+language,function(data,status)
                                      {
                                    //console.log(data);
                                    please_wait_modal.dialog('close');
                                    window.location = "../html/PatientDetails.php?IdUsu="+userId;
                                });
                            });
                        }
                        else
    ...
    $("#History").live('click',function() {
        ...
        var serviceUrl = '../../../ajax/getReportData.php?rawimage='+rawimage;
        //alert(query);
        //var RecTipo = LanzaAjax (cadena);
        ...
        var queUrl ='../../../ajax/getReportHistory.php?id='+pines[0].idpin;
        var queUrl1 ='../../../ajax/getReportViewers.php?id='+pines[0].idpin;
    ...
    $("#BotonAddClase").live('click',function() {
        ...
        if (name!=null && name!="")
        {
            ...
            var cadena = '../../../ajax/AnadeClase.php?queBlock='+queBlock+'&queUser='+queUsu+'&UltimoEvento='+UltimoEvento+'&Nombre='+name;
    ...
    $("#BotonElimClase").live('click',function() {
        ...
        if (r==true)
        {
            ...
            var queBlock = $("#queBlock").val();
            var UltimoEvento = $("#UltimoEvento").val();
            var cadena = '../../../ajax/EliminaClase.php?queBlock='+queBlock+'&queUser='+queUsu+'&UltimoEvento='+UltimoEvento+'&Nombre='+name;
            var RecTipo = LanzaAjax (cadena);
    ...
    $("#GrabaDatos").live('click',function() {
        ...
        if (r==true)
        {
            ...
            var cadena = '../../../ajax/GrabaClasif.php?queBlock='+queBlock+'&queUser='+queUsu+'&queERU='+queERU+'&queEvento='+queEvento+'&queTipo='+queTipo+'&idusfixed='+idusfixed+'&idusfixedname='+idusfixedname+'&Idmed='+idmed+'&fecha='+fecha;
            //alert (cadena);
    ...
    $("#BotonRecords").live('click',function() {
        ...
        var url = '../../../ajax/get_file_list.php?ElementDOM=na&EntryTypegroup=0&Usuario='+Usuario+'&MedID='+doctorId;
        //alert(url);
        var RecTipo = LanzaAjax (url);
        //alert(RecTipo);
        list = RecTipo.split(';');
    ...
    $(".TABES").live('click',function() {
        ...
        //if(EntryTypegroup!=10){
            queUrl1 = '../../../ajax/getNumThumbnails.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID; 
        //}
        ...
        var queUrl1='';
        if(EntryTypegroup==10){
            queUrl1 ='../../../ajax/createReportStreamLocked.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&num_reports='+num_reports+'&isDoctor='+isDoctor;
        }else{
            queUrl1 ='../../../ajax/createReportStreamNEWTEST.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&num_reports='+num_reports+'&isDoctor='+isDoctor;
            //console.log(queUrl);
        }
   ...
   $('#MorePage').live('click',function(){
    ...
    function get_more_reports(queid,offset)
    {
        ...
        var queUrl ='../../../ajax/createReportStreamDocGroupNEWTEST.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&offset='+offset+'&jump='+jump+'&isDoctor='+isDoctor;
    ...
    $("#BotonTestRS").live('click',function() {
        ...
        var queUrl ='../../../ajax/CreateReportStream.php?ElementDOM='+ElementDOM+'&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&isDoctor='+isDoctor;
            ...
            if(clicks === 1) {
                    ...
                    if(queBLD==null){
                        ...
                        if(quePEN==null){
                            ...
                            if(queDEL==null){
                                ...
                                {
                                    var cadena = '../../../ajax/DecryptFile.php?rawimage='+queImg+'&queMed='+doc_id;
                                    var RecTipo = LanzaAjax (cadena);
                        ...
                        */
                            ...
                            var queUrl ='../../../ajax/getTipoClase.php?BlockId='+queId;
                            ...
                            if(referral_state==2){
                                ...
                                var cadena='../../../ajax/getReportInformationReview.php?referralid='+referral_id;
                                //alert(cadena);
                                ...
                                if(parseInt(status)){
                                    ...
                                    var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id+'&stage=3';
                                    LanzaAjax(cadena);
                                    referral_state=3;
                                    $("div#infr_btn").append('<i style="margin-left:10px;font-size:15px" class="icon-arrow-right"></i>');
                                    var cadena='../../../ajax/push_server.php?message="Referral stage information review is completed"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+doc_id+'&channel='+other_doc;
                                    var RecTipo=LanzaAjax(cadena);
                                    var content="referal stage information review is completed";
                                    var subject="Referral stage information";
                                    var reportids=0;
                                    var cadena='../../../ajax/sendMessage.php?sender='+doc_id+'&receiver='+other_doc+'&patient='+mem_id+'&content='+content+'&subject='+subject+'&attachments='+reportids+'&connection_id='+referral_id;
                                    var RecTipo=LanzaAjax(cadena);
                ...
                if(queBLD==null){
                    ...
                    if(quePEN==null){
                        if(readwriteaccess==1){
                            $("#BotonMod").show();
                            var cadena = '../../../ajax/getprivacystatus.php?Idpin='+idpin+'&state=0+&type=0';
                            //alert(cadena);
                            var RecTipo = LanzaAjax (cadena);
                            //alert(RecTipo);
                            var normalprivate="";
                            ...
                            if(RecTipo=="normal"){
                                ...
                                if(normalprivate==true){
                                    //alert('normal!');
                                    var cadena = '../../../ajax/getprivacystatus.php?Idpin='+idpin+'&state=1+&type=1';
                                    var RecTipo = LanzaAjax (cadena);
                                    //alert (RecTipo);
                                }
                                ...
                                if(priv==true){
                                    var cadena = '../../../ajax/getprivacystatus.php?Idpin='+idpin+'&state=1+&type=0';
                                    var RecTipo = LanzaAjax (cadena);
                                    //alert (RecTipo);
                                }
    ...
    function upload(blob) {
        ...
        xhr.open("POST","../../../ajax/upload_file_audio.php?queId="+get_idusu+"&from="+doc_id,true);
        xhr.send(fd);
   ...
   $("#BotonUpload_New").live('click',function(){
	...
	var num = '';
        if (custom_look == "COL") {
		var url = "../../../ajax/get_num_reports.php";
		$.ajax({
		  type: 'POST',
		  url: url,
		  data: {id: $("#USERID").val()},
...
console.log('numrep:'+num);
    ...
    $("div[id^='reject_btn'] a").live('click',function(){
        ...
        if (option1)
        {
            ...
            setTimeout(function(){
                ...
                var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id_array[i]+'&stage=-1';
                salida = LanzaAjax(cadena);

                referral_state_array[i]=-1;

                var cadena='../../../ajax/push_server.php?message="Referral has been rejected!"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[i];
                var RecTipo=LanzaAjax(cadena);
                var content="The referral has been rejected";
                var subject="Referral stage information";
                var reportids=0;
                var cadena='../../../ajax/sendMessage.php?sender='+both_id+'&receiver='+otherdocid[i]+'&patient='+mem_id+'&content='+content+'&subject='+subject+'&attachments='+reportids+'&connection_id='+referral_id_array[i];
                var RecTipo=LanzaAjax(cadena);
                $("#H2M_Spin").hide();
    ...
    $("div[id^='ack_btn_pending'] a").live('click',function(){
        ...
        var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id_array[i]+'&stage=1&pending=1';
        //alert(cadena);
        var RecTipo=LanzaAjax(cadena);
        //alert(RecTipo);
        referral_state_array[i]=1;				
        displaynotification('status','You have acknowledged the referred patient!');
        var cadena='../../../ajax/push_server.php?message="Referral stage Acknowledgement is complete!"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[i];
        var RecTipo=LanzaAjax(cadena);
    ...
    $("div[id^='inpa_btn'] a").live('click',function(){
        ...
        if(referral_state_array[i]==0){
            ...
            if(referral_state_array[i]<3 && referral_state_array[i]!=-1)
                ...
                var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id_array[i]+'&stage=4';
                LanzaAjax(cadena);
                referral_state_array[i]=4;
                
                var cadena='../../../ajax/push_server.php?message="Referral stages has been completed"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[i];
                LanzaAjax(cadena);
    ...
    $("div[id^='infr_ref_btn'] a").live('click',function(){
        ...
        if(referral_state_array[i]==0){
            ...
            if(referral_state_array[i]>=2)
                ...
                var cadena='../../../ajax/getReportInformationReview.php?referralid='+referral_id_array[i];
                var status=LanzaAjax(cadena);
                var conf='false';
                if(status=='true'){
                    conf='true';
                ...
                if(conf){
                    ...
                    var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id_array[i]+'&stage=2';
                    LanzaAjax(cadena);
                    referral_state_array[i]=2;
                    $("div#infr_ref_btn"+i).append('<i style="margin-left:10px;font-size:15px" class="icon-arrow-right"></i>');
                    var cadena='../../../ajax/push_server.php?message="Referral stage information review is completed"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[i];
                    var RecTipo=LanzaAjax(cadena);
                    var content="referal stage information review is completed";
                    var subject="Referral stage information";
                    var reportids=0;
        ...
        if(referral_state_array[i]==0){
            ...
            if(referral_state_array[i]<2 && referral_state_array[i]!=-1)
                ...
                if(hassentmessage==0){
                    ...
                    $("div#cmnt_ref"+i).children("*").css("color","#3d93e0");
                    $(this).css("color","#3d93e0");
                    var cadena='../../../ajax/setReferralStage.php?referralid='+referral_id_array[i]+'&stage=3';
                    LanzaAjax(cadena);
        ...
        if(referral_state_array[i]==0){
            ...
            if(referral_state_array[i]>=2)
                ...
                if(conf){
                    ...
                    referral_state_array[i]=2;
                    $("div#app_btn"+i).append('<i style="margin-left:10px;font-size:15px" class="icon-arrow-right"></i>');
                    var cadena='../../../ajax/push_server.php?message="Referral stage appointment is completed"&NotifType=2&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[i];
                    LanzaAjax(cadena);
    ...
    $("[id^='newinbox']").live('click',function(){
        ...
        whichdocidclicked=i;
        //alert (i);
        var queUrl ='../../../ajax/getInboxMessage.php?IdMED='+both_id+'&patient='+mem_id+'&sendingdoc='+otherdocid[i];
        //var RecTipo=LanzaAjax(queUrl);
        //alert (RecTipo);
        $("#datatable_3_"+i).load(queUrl);
    ...
    $("[id^='newoutbox']").live('click',function(){
        ...
        whichdocidclicked=i;
        //alert (i);
        var queUrl ='../../../ajax/getOutboxMessage.php?IdMED='+both_id+'&patient='+mem_id+'&receivingdoc='+otherdocid[i];
    ...
    $('.CFILA').live('click',function(){
        ...
        if(reportsattached){
            ...
            var queUrl ='../../../ajax/createAttachmentStreamNEWTEST.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&Reports='+reportsattached;
            var RecTipo=LanzaAjax(queUrl);
            //alert(RecTipo);
        ...
        //$('#clearmessage').hide();
        $('#message_modal').trigger('click');
        var cadena='../../../ajax/setMessageStatus.php?msgid='+id;
        var RecTipo=LanzaAjax(cadena);
        setTimeout(function(){
    ...
    $('#sendmessages').live('click',function(){
        ...
        if(subject==''||content=='')
            ...
            var cadena='../../../ajax/sendMessage.php?sender='+both_id+'&receiver='+otherdocid[whichdocidclicked]+'&patient='+mem_id+'&content='+content+'&subject='+subject;
            var RecTipo=LanzaAjax(cadena);
            //alert(RecTipo);
            ...
            var cadena='../../../ajax/push_server.php?message="message from a doctor"&NotifType=1&IdUsu='+get_idusu+'&FromDoctorId='+both_id+'&channel='+otherdocid[whichdocidclicked];
            var RecTipo=LanzaAjax(cadena);
        }
    ...
    $('#sendmessages_inbox').live('click',function(){
	  ...
	  displaynotification('Error','Error sending message.Empty subject or content area!');
        ...
        var cadena='../../../ajax/sendMessage.php?sender='+both_id+'&receiver='+otherdocid[whichdocidclicked]+'&patient='+mem_id+'&content='+content+'&subject='+subject+'&attachments='+reportids+'&connection_id='+referral_id+'&type=patientdetailmed';
            ...
            $('input[type=checkbox][id^="checkcol"]').each(
                function () {
                    ...
                    if(sThisVal==1){
                        ...
                        //messageid=messageid+idp+' ,';
                        num=num+1;
                        var cadena='../../../ajax/setMessageStatus.php?msgid='+idp+'&delete=1';
                        LanzaAjax(cadena);
                    }
   ...
   //$("div.ichat").show();
    ...
    $("div[id^='notif_']").live('click',function(){
        ...
        var cadena ='../../../ajax/setNotificationStatus.php?Id='+id;
        var status=LanzaAjax(cadena);
        //alert(status);
        //$("div[id^='notif']").remove();  
        $(this).remove();   
   ...
   $('#datatable_3').trigger('update');*/
    ...
    var cadena ='../../../ajax/getNotificationCount.php?IdMED='+both_id;
    ...
    $("a#notification_bar").live('click',function(e){
        ...
        $("a#notification_bar").toggle(
            function(){
                ...
                //var notify_num=78;
                var queUrl ='../../../ajax/getNotificationMessages.php?IdMED='+both_id;
                //
 ...
 */ 
    ...
    function getReportCreator(Idpin,Iddoc,Idusu) {
        ...
        var cadenaGUD = '../../../ajax/getReportCreator.php?Idpin='+Idpin+'&Iddoc='+Iddoc+'&Idusu='+Idusu;
        //alert(cadenaGUD);
        $.ajax(
            {
                url: cadenaGUD,
...
function next_click()
{
    ...
    {
        var cadena = '../../../ajax/DecryptFile.php?rawimage='+file_name+'&queMed='+both_id;
        //alert(cadena);
        var RecTipo = LanzaAjax (cadena);
        //alert(RecTipo);
        var doctorId = -1;

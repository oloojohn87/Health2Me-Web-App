...
else
{
    ...
    else
    {
		...
		$q->bindValue(2, $ip, PDO::PARAM_STR);
		$q->execute();
        header( 'Location: ../../ajax/double_login.php' ) ;
        //$q = "insert into  session_log values(".$_SESSION['MEDID'].",'Login',now(),'".$ip."')";
        //mysql_query($q);
    }
}
...
<head>
    ...
    <link rel='stylesheet' href='../../master_css/bootstrap-dropdowns.css'>
        <style>
            .addit_button{
            ...
            <div class="dropdown-menu" id="prof_dropdown" style="background-color:transparent; border:none; -webkit-box-shadow:none; box-shadow:none;">
                <div class="item_m"><span class="caret"></span></div>
                    <ul class="clear_ul" >
                        <li>
                            <?php 
                                if ($privilege==1) echo '<a href="../../provider/maindashboard/html/MainDashboard.php">';
                                else if($privilege==2) echo '<a href="../member/patientdetails/html/PatientDetails.php">';
                            ?>
                        <i class="icon-globe"></i> Home</a></li>
                    
                        
                        <li><a href="../../ajax/logout.php"><i class="icon-off"></i> Sign Out</a></li>
                    </ul>
                </div>
            </div>
            <!--Button User END-->  
     ...
     <div class="speedbar">
        <div class="speedbar-content" style="position:relative;">
            <ul class="menu-speedbar">
                <li><a href="../userdashboard/html/UserDashboard.php">Dashboard</a></li>
                <!--li><a href="patients.php" >Members</a></li-->

                <li><a href="../../ajax/logout.php" style="color:yellow;">Sign Out</a></li>
            </ul>
...
<script src="../../modules/TypeWatch/jquery.typewatch.js"></script>
<script src="../../master_js/spin.js"></script>
<!--script src="realtime-notifications/lib/gritter/../../master_js/jquery.gritter.min.js"></script>
...
$(document).ready(function() 
{
    ...
    $('#AttachmentButton').live('click',function(e){
        ...
        {
            $("#attachments").empty();
            var queUrl ='../../ajax/createAttachmentStreamNEW.php?ElementDOM=na&EntryTypegroup=0&Usuario='+current_message.SENDER_ID+'&MedID='+$("#MEDID").val()+'&IGNPR=1&Reports='+current_message.ATTACHMENTS;
            console.log(queUrl);
            ...
            $(".attachments").on('click', function(e)
            {
                ...
                console.log(file);
                $.get('../../ajax/DecryptFile.php?rawimage='+file+'&queMed='+$("#MEDID").val(), function(data, status)
                {
    ...
    $('#SendButton').live('click',function(e){
        ...
        else
        {
            ...
            $.post("../../ajax/updateMessage.php", {mes: 0, scenario: type, type: 'send', IdMED: $("#MEDID").val(), IdRECEIVER: sen_id, SUBJECT: $("#reply_subject").val(), MESSAGE: $("#reply_content").val()}, function(data, status){});
    ...
    $('#ComposeSendButton').live('click',function(e){
        ...
        else
        {
            var type = 'patient';
            if($('#Type_toggle').is(":checked")/* || folder == 2*/) type = 'doctor';
            $.post("../../ajax/updateMessage.php", {mes: 0, scenario: type, type: 'send', IdMED: $("#MEDID").val(), IdRECEIVER: compose_selected_user, SUBJECT: $("#compose_message_subject").val(), MESSAGE: $("#compose_message_content").val()}, function(data, status){
				alert("Message sent");
				});
    ...
    $('#delete_button').live('click',function(e){
        var type = 'patient';
        if($('#Type_toggle').is(":checked")/* || folder == 2*/) type = 'doctor';
        $.post("../../ajax/updateMessage.php", {mes: selected, status: 'read', scenario: type, type: 'delete', IdMED: $("#MEDID").val()}, function(data, status){GetMessages(folder, '');});
        e.preventDefault();
    });
    ...
    $('#search_users_bar_button').live('click',function(e){
        if($("#search_users_bar").val().length > 0)
        {
            ...
            $.post("../../ajax/getUsersSearch.php", {search: $("#search_users_bar").val(), type: type, doc: $("#MEDID").val()}, function(data, status)
            {
    ...
    $('#mark_unread_button').live('click',function(e){
        if(folder == 1)
        {
            var type = 'patient';
            if($('#Type_toggle').is(":checked")/* || folder == 2*/) type = 'doctor';
            $.post("../../ajax/updateMessage.php", {mes: selected, status: 'new', scenario: type, type: 'unread', IdMED: $("#MEDID").val()}, function(data, status){GetMessages(folder, '');});
        }
        else
        {
    ...
    $("#subject_title a").live('click',function(e){
        ...
        if(folder == 1)
        {
            ...
            $.post("../../ajax/updateMessage.php", {mes: messages[this.id].ID, status: 'read', type: 'read', scenario: type, IdMED: $("#MEDID").val()}, function(data, status){});
        }
        
        $('#message_modal').dialog({bgiframe: true, width: 1000, height: 500, modal: false, resizable: false});
        if(type == 'patient')
        {
            $("#messagedetail_from").html('<a style="color: #54bc00;" href="../patientdetails/html/PatientDetails.php?IdUsu=' + messages[this.id].SENDER_ID + '">' + messages[this.id].SENDER + '</a>');
        }
        else
        {
    ...
    function LoadSingleMessage(mes)
    {
        ...
        var type = 'patient';
        if($('#Type_toggle').is(":checked")/* || folder == 2*/) type = 'doctor';
        $.post("../../ajax/updateMessage.php", {mes: mes, status: 'read', type: 'read',scenario: type, IdMED: $("#MEDID").val()}, function(data, status){GetMessages(folder, '');});
        $.post("../../ajax/getInboxMessageSingle.php", {id: mes, scenario: type}, function(data, status)
		{
            ...
            if(type == 'patient')
            {
                $("#messagedetail_from").html('<a style="color: #54bc00;" href="../patientdetails/html/PatientDetails.php?IdUsu=' + item.ID + '">' + item.SENDER + '</a>');
            }
            else
            {
    ...
    function GetMessages(t, search)
	{
        ...
        var type = 'patient';
        if($('#Type_toggle').is(":checked")/* || folder == 2*/) type = 'doctor';
        var file = "../../ajax/getInboxMessageAll.php";
        if(t == 2)
        {
            file = "../../ajax/getSentMessageAll.php";
        }
        $.post(file, {IdMED: queMED, scenario: type, page: current_page, search: search}, function(data, status)
		{
            ...
            if(items.length > 0)
            {
                ...
                for(var i = 0; i < items.length; i++)
                {
                    ...
                    if(type === 'patient')
                    {
                        html_str += '<a style="color: #54bc00;" href="../patientdetails/html/PatientDetails.php?IdUsu=' + items[i].ID_pat + '">';
                    }

...
while($row = $result->fetch(PDO::FETCH_ASSOC))
{
    ...
    {
        decrypt_files($row['RawImage'],$MedID,$pass);
        $selecURL ='temp/'.$MedID.'/PackagesTH_Encrypted/'.$ImageRaiz.'.'.$extensionR;
        //echo $selecURL;
        //Adding changes to handle jpg files. It seems in some channel the jpg file are getting converted as png
        if(file_exists('temp/'.$MedID.'/PackagesTH_Encrypted/'.$ImageRaiz.'.'.$extensionR))
        {
    ...
    else 
    {
        if	($row['CANAL']==7)
        {
            ...
            $selecURL ='temp/'.$queUsu.'/PackagesTH_Encrypted/'.$row['RawImage'];
            $selecURLAMP ='temp/'.$queUsu.'/Packages_Encrypted/'.$row['RawImage'];
        ...
        else 
        {
            ...
            { 
                $selecURL ='eMapLife/PinImageSetTH/'.$row['RawImage']; 
            }
				// COMPROBACIÓN DE EXISTENCIA DEL ARCHIVO (PARA LOS CASOS DE EMAPLIFE iOS o ANDROID QUE TODAVIA NO GENERAN THUMBNAILS Y NO REFERENCIAN AL DIRECTORIO -TH
            $file = $selecURL;
            $file_headers = @get_headers($file);
    ...
    $inyectHTML .=' 
         ...
         <div class="attachments" style="width:150px; height:250px; float:left; margin-right:10px; margin-bottom:20px; -webkit-box-shadow: 3px 3px 14px rgba(25, 25, 25, 0.5); -moz-box-shadow:3px 3px 14px rgba(25, 25, 25, 0.5); box-shadow:3px 3px 14px rgba(25, 25, 25, 0.5);">
           	<div style="border: 1px solid blue; background-color: RGB(200, 200, 200); height:50px; width:148px; border: 1px solid RGB(223,223,223);">
			    <p class="CheckContainer" style="float:left; height:30px; width:20px; padding:0px; margin:0px; margin-top:5px; margin-left:10px;"><span><input type="checkbox" name="numbers[]" class="mc" value="0" id="reportcol'.'_'.$index.'_'.$row['IdPin'].'" ';
    if(strpos($row['orig_filename'], "Summary") !== false || strpos($row['orig_filename'], "EMR") !== false)
    {
        $inyectHTML .= 'checked disabled';
    }
    $inyectHTML .= '/><label for="reportcol'.'_'.$index.'_'.$row['IdPin'].'"><span></span></label></span><i class="'.$TipoIcon[$indi].'" icon-large" style="color:RGB(111,111,111); font-size: 1.0em;"></i></p>
		  		<p class="queTIP" style="float:left; height:20px; width:90px; padding:0px; margin:0px; margin-top:0px; margin-left:10px; color:RGB(65,65,65); font-size:12px; font-family: "Myriad Pro", Myriad, "Liberation Sans", "Nimbus Sans L", "Helvetica Neue", Helvetica, Arial, sans-serif; overflow: hidden;" >'.substr($Tipo[$indi],0,12).'</p>
    ...
    $inyectHTML .= '<div class="queIMG" style="background-color: white; height:150px; width:150px;  padding:0px; margin:0px; overflow: hidden;">';
...
$PREinyectHTML = '<div style="width: '.$WidthStream.'px; height: 270px; margin-top:10px; margin-left:10px;"><div style="overflow-x: auto; overflow-y: hidden;">';

$inyectHTML = '';
$inyectHTML = $PREinyectHTML.$PARTIALinyectHTML.'</div></div>';

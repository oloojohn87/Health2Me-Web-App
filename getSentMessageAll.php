...
if ($scenario == 'patient') 
{
    $setQuery = $con->prepare("SELECT * FROM message_infrastructureuser WHERE receiver_id=? AND tofrom='from' ORDER BY fecha DESC");
	$setQuery->bindValue(1, $IdMED, PDO::PARAM_INT);
	
}
else
{
    $setQuery = $con->prepare("SELECT * FROM message_infrasture WHERE sender_id=? ORDER BY fecha DESC");
	$setQuery->bindValue(1, $IdMED, PDO::PARAM_INT);
}

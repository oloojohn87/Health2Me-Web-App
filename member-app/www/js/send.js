$( document ).ready(function() {  
		...
		$('#send-doc').click(function() {
			...
			var doctor_email = $('#doctor-email input').val();
        	var idpins_arr = [];
        	var idpins_count = 0;
			
			//find the ids of the choosen files
			$("#droppable").children().each(function(n, i) {
  					var id = this.id;
			...
			var idpins = idpins_arr.join("_");

			//this file doesn't have header in it to return data	
        	$.get(domain+"mobile-member/ValidateSendEmailDoc.php?emailId="+doctor_email+"&user="+user+"&idpins="+idpins,function(data,status)
              {
 			    var UserID = user;  		
                var queUrl = domain+'mobile-member/GetDoctorId.php?doctoremail='+doctor_email;
                var doctorid = LanzaAjax(queUrl);
                var queUrl = domain+'mobile-member/AddUserActivity.php?userid='+UserID+'&doctorid='+doctorid+'&type=1&status=1';
                var result = LanzaAjax(queUrl);
                //alert('The records you selected have been sent to your chosen doctor.');
				
                console.log(data);
				$('#doctor-email input').val('');
				$("#droppable .note").replaceWith('');	
				$("#start-text").show();
				$("#start-text").html("Your records have been sent.");
...
/*In: Group												*/
/*Out : EntryTypeGroup									*/
/*Desc: get medical history reports						*/
/********************************************************/

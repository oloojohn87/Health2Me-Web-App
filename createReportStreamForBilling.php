...
if($isDoctor==1){
      ...
      if($hasfullaccess==1){
           ...
           if($EntryTypegroup==0)
         {
            ...
            if($EntryTypegroup==5){
             ...
             $query=$con->prepare("select q.* from ((select LP.* from lifepin LP INNER JOIN ((select A.idDoctor from doctorsgroups A INNER JOIN (select idGroup from doctorsgroups where idDoctor=?) B where B.idGroup=A.idGroup) UNION (select A.idDoctor from doctorsgroups A INNER JOIN (select idGroup from doctorsgroups grp INNER JOIN (select IdMED from doctorslinkdoctors where IdMED2=? and IdPac=?)rd where grp.idDoctor=rd.IdMed) B where B.idGroup=A.idGroup) UNION (select Id from doctors where Id=?) UNION (select IdMED from doctorslinkdoctors where IdMED2=? and IdPac=?)) AB where LP.IdMed=AB.idDoctor and IdUsu=? and (LP.markfordelete=0 or LP.markfordelete is null) and (LP.IsPrivate=0 or LP.IsPrivate is null) and NOT (LP.Tipo IN (select Id from tipopin where Agrup=9) and LP.IdMED!=?) and LP.emr_old=0 ".$limitString." ) UNION (select * from lifepin where (IdMED=0 or IdMED IS NULL) and IdUsu=? and (markfordelete=0 or markfordelete is null) and (IsPrivate=0 or IsPrivate is null) and emr_old=0 ".$limitString." )UNION (select lp.* from lifepin lp INNER JOIN (select IdPin from doctorslinkusers where IdMED=? and IdUs=? and IdPIN IS NOT NULL) dlu where dlu.IdPIN=lp.IdPin and emr_old=0 ".$limitString." )) q ORDER BY q.fecha DESC LIMIT ".$limit);
    ...
    if($EntryTypegroup==0)
        {
            $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? AND bill_id = ? and emr_old=0 and (markfordelete=0 or markfordelete is null) and Tipo NOT IN (select Id from tipopin where Agrup=9) ORDER BY Fecha DESC ".$limitString);
        
            $query->bindValue(1, $queUsu, PDO::PARAM_INT);
            $query->bindValue(2, $bill_id, PDO::PARAM_INT);
        }
        else
        {
            $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? AND bill_id = ? and emr_old=0 and (markfordelete=0 or markfordelete is null) and Tipo NOT IN (select Id from tipopin where Agrup=9) ".$limitString." ORDER BY Fecha DESC LIMIT ".$limit);
            $query->bindValue(1, $queUsu, PDO::PARAM_INT);
            $query->bindValue(2, $bill_id, PDO::PARAM_INT);
        }
			 	...
			 	if(!in_array($row['IdPin'], $PendingReportID)){
					if(!in_array($row['IdPin'], $deletedreports)){
                        if($row['isDicom'] == 1)
                        {
							...
							$inyectHTML .= '<img src="'.$damain.'/temp/DICOM_svg.png" alt="'.$hasReadWriteAccess.'" style="margin-top:0px;">'; 
							$inyectHTML .= '</div>';
                        }
						if($extensionR=='wav'){

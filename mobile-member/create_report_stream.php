...
if($isDoctor==1){
            ...
            // Check if the doctor is the creator of the patient by checking the creator field
              ...
              $q=$con->prepare("Select IdCreator from usuarios USU INNER JOIN (select distinct(B.idDoctor) from monimed.doctorsgroups B INNER JOIN (select A.idGroup,A.idDoctor from monimed.doctorsgroups A where A.idDoctor=?) C where B.idGroup=C.idGroup) DG  where USU.Identif=? and DG.idDoctor=USU.IdCreator");
              ...
              if($hasfullaccess!=1){
                ...
                $q=$con->prepare("select IdMED2 from monimed.doctorslinkdoctors where IdMED2=? and IdPac=? and estado=2");
                ...
                $q=$con->prepare("select IdPin from monimed.lifepin where IdMed=? and IdUsu=? LIMIT 1");
             ...
             if($hasfullaccess!=1){
                 ...
                 $q=$con->prepare("(Select IdPIN from doctorslinkusers DLU INNER JOIN (select distinct(B.idDoctor) from monimed.doctorsgroups B INNER JOIN (select A.idGroup,A.idDoctor from monimed.doctorsgroups A where A.idDoctor=?) C where B.idGroup=C.idGroup) DG where DG.idDoctor=DLU.IdMed and DLU.IdUs=?)UNION(Select IdPIN from doctorslinkusers where IdMed=? and IdUs=?)");
      ...
      if($hasfullaccess==1){
           ...
           if($EntryTypegroup==0)
         {
                 ...
                 $query=$con->prepare("select q.* from ((select LP.* from monimed.lifepin LP INNER JOIN ((select A.idDoctor from monimed.doctorsgroups A INNER JOIN (select idGroup from monimed.doctorsgroups where idDoctor=?) B where B.idGroup=A.idGroup)UNION (select A.idDoctor from monimed.doctorsgroups A INNER JOIN (select idGroup from monimed.doctorsgroups grp INNER JOIN (select IdMED from monimed.doctorslinkdoctors where IdMED2=? and IdPac=?)rd where grp.idDoctor=rd.IdMed) B where B.idGroup=A.idGroup) UNION (select Id from monimed.doctors where Id=?) UNION (select IdMED from monimed.doctorslinkdoctors where IdMED2=? and IdPac=?)) AB where LP.IdMed=AB.idDoctor and IdUsu=? and (LP.markfordelete=0 or LP.markfordelete is null) and (LP.IsPrivate=0 or LP.IsPrivate is null) and NOT (LP.Tipo IN (select Id from monimed.tipopin where Agrup=9) and LP.IdMED!=?) and LP.emr_old=0) UNION (select * from monimed.lifepin where (IdMED=0 or IdMED IS NULL) and IdUsu=? and (markfordelete=0 or markfordelete is null) and (IsPrivate=0 or IsPrivate is null) and emr_old=0 ) UNION (select lp.* from monimed.lifepin lp INNER JOIN (select IdPin from monimed.doctorslinkusers where IdMED=? and IdUs=? and IdPIN IS NOT NULL) dlu where dlu.IdPIN=lp.IdPin )) q ORDER BY q.fecha DESC ".$limitString);
            ...
            if($EntryTypegroup==5){
                ...
                $query=$con->prepare("SELECT * FROM lifepin WHERE IdPin IN ($IdPins) ".$limitString." ORDER BY Fecha DESC");
            
            
            }else{
             $query=$con->prepare("select q.* from ((select LP.* from monimed.lifepin LP INNER JOIN ((select A.idDoctor from monimed.doctorsgroups A INNER JOIN (select idGroup from monimed.doctorsgroups where idDoctor=?) B where B.idGroup=A.idGroup) UNION (select A.idDoctor from monimed.doctorsgroups A INNER JOIN (select idGroup from monimed.doctorsgroups grp INNER JOIN (select IdMED from monimed.doctorslinkdoctors where IdMED2=? and IdPac=?)rd where grp.idDoctor=rd.IdMed) B where B.idGroup=A.idGroup) UNION (select Id from monimed.doctors where Id=?) UNION (select IdMED from monimed.doctorslinkdoctors where IdMED2=? and IdPac=?)) AB where LP.IdMed=AB.idDoctor and IdUsu=? and (LP.markfordelete=0 or LP.markfordelete is null) and (LP.IsPrivate=0 or LP.IsPrivate is null) and NOT (LP.Tipo IN (select Id from monimed.tipopin where Agrup=9) and LP.IdMED!=?) and LP.emr_old=0 ".$limitString." ) UNION (select * from monimed.lifepin where (IdMED=0 or IdMED IS NULL) and IdUsu=? and (markfordelete=0 or markfordelete is null) and (IsPrivate=0 or IsPrivate is null) and emr_old=0 ".$limitString." )UNION (select lp.* from monimed.lifepin lp INNER JOIN (select IdPin from monimed.doctorslinkusers where IdMED=? and IdUs=? and IdPIN IS NOT NULL) dlu where dlu.IdPIN=lp.IdPin and emr_old=0 ".$limitString." )) q ORDER BY q.fecha DESC LIMIT ".$limit);
    ...
    if($EntryTypegroup==0)
        {
            $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 and (markfordelete=0 or markfordelete is null) and Tipo NOT IN (select Id from tipopin where Agrup=9) ORDER BY Fecha DESC ".$limitString);
        ...
        {
            $query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 and (markfordelete=0 or markfordelete is null) and Tipo NOT IN (select Id from tipopin where Agrup=9) ".$limitString." ORDER BY Fecha DESC LIMIT ".$limit);
            $query->bindValue(1, $queUsu, PDO::PARAM_INT);
           // $query->bindValue(2, $MedID, PDO::PARAM_INT);
        }
}	
...
$sql_r=$con->prepare("(select distinct(B.idDoctor) from monimed.doctorsgroups B INNER JOIN (select A.idGroup,A.idDoctor from monimed.doctorsgroups A where A.idDoctor in (select idcreator from lifepin where idpin=?)) C where B.idGroup=C.idGroup) UNION (select idcreator from lifepin where idpin=?)");
			 	...
			 	if(!in_array($row['IdPin'], $PendingReportID)){
					if(!in_array($row['IdPin'], $deletedreports)){
                                    ...
                                    $inyectHTML .=' 
                                             ...
                                             <div class="note ui-widget-content" id="'.$row['IdPin'].'" style="">
                                                <div class="note-header">
                                                    <p class="note-header-text" ><i class="'.$TipoIcon[$indi].'" icon-large" style=""></i></p>

                                                    <p class="queTIP" id="'.$queTIPValue.'" >'.$queTIPValue.'</p>
                                                    <p class="date '.$FechaBien.'" style="">'.$FechaBien.'</p>		

                                                    <span class="label label-info label-style" style="background-color:'.$TipoColor[$indi].'">'.$TipoAB[$indi].'</span>
	 ...
	 if(!in_array($row['IdPin'], $blindReportId)){
			 	if(!in_array($row['IdPin'], $PendingReportID)){
					if(!in_array($row['IdPin'], $deletedreports)){
                        if($row['isDicom'] == 1)
                        {
                            $inyectHTML .= '<div class="queIMG" id="'.$row['RawImage'].'" style="">';
							$inyectHTML .= '<img src="/temp/DICOM_svg.png" alt="'.$hasReadWriteAccess.'" style="margin-top:0px;">'; 
							$inyectHTML .= '</div>';
                        }
						if($extensionR=='wav'){
						
							$inyectHTML .= '<div class="queIMG" id="'.$row['RawImage'].'">';
						...
						}else if($extensionR=="MOV")
						{
							...
							$inyectHTML .= '<div class="queIMG" id="'.$row['RawImage'].'" style="background-color: white; padding:0px; margin:0px; overflow: hidden;">';
		...
		$inyectHTML .= '  <div class="note-footer" style="background-color: '.$TipoColor[$indi].';">';
...
$PREinyectHTML = '<div id="ascroll" style="width: '.$WidthStream.'px; margin-top:10px; margin-left:10px;overflow-y:hidden">';
...
function decrypt_files($rawimage,$queMed,$pass )
{
	...
	else 
	{
		...
		//DEBUG:10/24/2014 LS
		$command = "echo '".$pass."' | openssl aes-256-cbc -pass stdin -d -in ../PackagesTH_Encrypted/".$ImageRaiz.".".$extension." -out ../temp/".$queMed."/PackagesTH_Encrypted/".$ImageRaiz.".".$extension."<br>";
		//echo $command;
		//echo "Image : ".$ImageRaiz."<br>";
...
/****************************************************/
/*IN: UserID - User identifier						*/
/*OUT: Returns nothing								*/
/*DESC: Function that initializes the server for 	*/
/*		med-history thumnails						*/
/****************************************************/
function initializeTemp($UserID) {

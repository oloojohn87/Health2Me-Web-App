...
<!--  <body onload="$('.note').trigger('click'); $('.TABES').children().trigger('click');"> -->
  <body onload=" $('.TABES:eq(<?php echo $tabToOpen;?>)').click();">
      ...
      <!--  Start of comment <div style="float:left;">
		   ...
		   <div class="btn-group pull-right" >
            ...
            <a class="btn btn-profile dropdown-toggle" id="button-profile" data-toggle="dropdown" href="#">
              <span class="name-user"><strong lang="en">Welcome</strong>, <?php if($MedID > 0){echo "Dr. ".$IdMEDName." ".$IdMEDSurname;} else{ echo $MedUserName.' '.$MedUserSurname;} ?></span> 
             <?php 
            ...
            <ul class="clear_ul" >
                ...
                <?php } else { ?>
                <li><a href="UserDashboard.php" lang="en"><i class="icon-globe"></i> Dashboard</a></li>
                <?php } ?>
    ...
    <div id="connectMemberDialog" title="Connect New Member" style="width: 600px; height: 600px; display: none">
        <div id="connectMemberStep0" style="width: 100%; height: 460px; margin-top: 20px; display: block;">
            <span style="color: #54BC00; font-size: 18px;">Select Appointment</span>
            <style>
                .connectMemberRow{
                    width: 486px; 
                    height: 38px;
            ...
            <div class="controls" style="width: 500px; margin: auto; margin-top: 20px;margin-bottom: 8px;">
                <input id="selectAppointmentDate" style="float: left; width: 230px; border-top-right-radius: 0px; border-bottom-right-radius: 0px;" type="date">
                <input class="ui-timepicker-input" autocomplete="off" id="selectAppointmentTime" style="float: left; width: 130px; border-top-right-radius: 0px; border-bottom-right-radius: 0px;" type="text">
                <input onclick="checkNPAvailability();" class="btn btn-default selectAppointmentButton" id="selectAppointmentButton" lang="en" type="button" value="Check Availability" />
            </div>
            <div id="selectedNursePractitioner" style="border-radius: 5px; width: 500px; height: 400px; margin: auto; overflow: hidden; overflow-y: auto; margin-top: 15px;">
        ...
        <div class="clearfix" style="margin-bottom:20px;"></div>
		 ...
		 <!-- image upload changes start -->
		  ...
		  ?>	
                ...
                <?php
                    $DLUquery = $con->prepare("SELECT IdUs FROM doctorslinkusers WHERE IdMED = ? AND IdUs = ?");           
                    $DLUquery->bindValue(1, $IdMed, PDO::PARAM_INT);
                    $DLUquery->bindValue(2, $IdUsu, PDO::PARAM_INT);
                    $DLUquery->execute();
                    $DLUq = $DLUquery->rowCount();

                    $DLDquery = $con->prepare("SELECT DLD.IdPac FROM doctorslinkdoctors DLD 
                    INNER JOIN doctorsgroups DG1 ON DLD.IdMED2 = DG1.idDoctor 
                    INNER JOIN doctorsgroups DG2 ON DG1.idGroup = DG2.idGroup 
                  WHERE DG2.idDoctor = ? AND IdPac=?");           
                    $DLDquery->bindValue(1, $IdMed, PDO::PARAM_INT);
                    $DLDquery->bindValue(2, $IdUsu, PDO::PARAM_INT);
                    $DLDquery->execute();
                    $DLDq = $DLDquery->rowCount();
                    
                    if($DLUq > 0 && !$DLDq) echo '<span id="share_group"><button id="share_pat_group" class="btn btn-success">Share this patient to your group</button></span>';
...
if (isset($_SESSION['CustomLook']) && $_SESSION['CustomLook']!='COL') {
        ...
        <div class="label label-success" style="left:0px; margin-left:0px; margin-top:20px; margin-bottom:5px; font-size:14px; padding: 7px; width: 97%; text-align: center; margin-bottom: -20px;" l<?php
session_start();
set_time_limit(180);
require("environment_detail.php");
require("push_server.php");

...
####changes from cloud channel---Starts
	//for ($i = 0; $i < $size; $i++) {
					...
					$filepath=$domain."Packages_Encrypted/".$new_image_name;
					
					$Tamano=$_FILES["file"]["size"];

                    try{ if ($Tamano > 1024*1024*10) 
				    ...
				    //echo md5_file($filename);
				        if(strtolower($extension)=="doc"){
                            ...
                            $docpath=$domain."Document_Processing/docs/".$new_image_name_doc;
                            $resultado = move_uploaded_file($_FILES["file"]["tmp_name"], $docpath);
                            $isaDocument=1;
                        }else{
                            $extensions_list = array('doc', 'docx', 'png', 'jpg', 'tiff', 'gif', 'pdf');
                            
                            try { if(!in_array(strtolower($extension), $extensions_list)) throw new Exception("ExtError"); }
                            catch (Exception $e) { 
                                echo 'ExtError';
                                die();  
                            }
					...
					if($format){
					 $Isql="INSERT INTO lifepin SET NeedACTION = 1, IdEmail='1', IdUsu=$IdUsu, FechaInput=NOW(), Fecha='$reportdate' , FechaEmail = Now() , IdUsFIXED='$IdUsFIXED', IdUsFIXEDNAME='$IdUsFIXEDNAME', IdMedEmail = '$fromname', CANAL=5, ValidationStatus=99, EvRuPunt= 2, Evento=99, Tipo=$reporttype, fs=1, checksum='$checksum', hide_from_member='$hide_from_member'";
					 echo "<br>\n"; 	 
					 //KYLEecho $Isql;
					}else {
					 $Isql="INSERT INTO lifepin SET NeedACTION = 1, IdEmail='1', FechaInput=NOW(), Fecha='$reportdate' , FechaEmail = Now() , IdUsFIXED='$IdUsFIXED', IdUsFIXEDNAME='$IdUsFIXEDNAME', IdMedEmail = '$fromname', CANAL=5, ValidationStatus=99, EvRuPunt= 2, Evento=99, Tipo=$reporttype,checksum='$checksum', hide_from_member='$hide_from_member'";
					 echo "<br>\n"; 
					 //KYLEecho $Isql;	 
					}
					...
					if(strtolower($extension) == "jpe" || strtolower($extension) == 'jpeg'){
 						$extension="jpg";
						}
                        ...
                        if($extension=="doc" || $isaDocument==1){
                            ...
                            $Isql="UPDATE lifepin SET IdEmail='1', FechaInput=NOW(), ValidationStatus=8 , orig_filename ='$filename',fs=0,idusu=".$queId.",idcreator=".$_SESSION['BOTHID']." WHERE IdPin='$IdPin'";
						    $q = mysql_query($Isql);
						...
						// GRABACIÓN FÍSICA DEL FICHERO CONTENIDO EN EL ATTACHMENT EN EL SERVIDOR (PACKAGES)
 						...
 						$locFile = $local."Packages_Encrypted";
						...
						$locFileTH = $local."PackagesTH_Encrypted";
                        ...
                        shell_exec("cp ".$local."temp/".$new_image_name." Packages_Encrypted/");
						...
						$Isql="UPDATE lifepin SET IdEmail='1', RawImage='$new_image_name' , FechaInput=NOW(), ValidationStatus=8 , orig_filename ='$filename',fs=1,idusu=".$queId.",idcreator=".$_SESSION['BOTHID']." WHERE IdPin='$IdPin'";
						$q = mysql_query($Isql);
						$IdPin = mysql_insert_id();

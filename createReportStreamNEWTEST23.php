...
if($isDoctor==1){
    ...
    if($hasfullaccess!=1){
        //echo 'FULL ACCESS 2';
        $q=$con->prepare("Select IdCreator from usuarios USU 
                            INNER JOIN (select distinct(B.idDoctor) from doctorsgroups B 
                                INNER JOIN (select A.idGroup,A.idDoctor from doctorsgroups A where A.idDoctor=?) C 
                            ON B.idGroup=C.idGroup) DG 
                          ON USU.Identif=? and DG.idDoctor=USU.IdCreator");
    ...
    if($hasfullaccess!=1){
        //echo 'FULL ACCESS 3';
        $q=$con->prepare("SELECT IdPac FROM doctorslinkdoctors WHERE IdMED2 = ? AND IdPac = ? AND estado = 2
                            UNION
                          SELECT DLD.IdPac FROM doctorslinkdoctors DLD
                            INNER JOIN doctorsgroups DG1 ON DLD.IdMED2 = DG1.idDoctor 
                            INNER JOIN doctorsgroups DG2 ON DG1.idGroup = DG2.idGroup 
                          WHERE DG2.idDoctor = ? AND DLD.IdPac=? AND DLD.estado = 2");
...
//if($num_reports)
//{
	//$WidthStream = $num_reports * 150 + (count($highlightedReports)*5);
//}
//else
//{

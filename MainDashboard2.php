...
if ($_SESSION['CustomLook']=='COL') {
    header('Location:MainDashboard_hti.php'); 
...
if($count==1){
    ...
    // get all doctors that are currently in a phone consultation
            ...
            <div class="dropdown-menu" id="prof_dropdown">
                <div class="item_m"><span class="caret"></span></div>
                    <ul class="clear_ul" >
                        <li>
                            <?php 
                                if ($privilege==1) echo '<a href="dashboard.php" lang="en">';
                                else if($privilege==2) echo '<a href="patients.php" lang="en">';
                            ?>
     ...
     <div class="speedbar">
        <div class="speedbar-content" style="position:relative;">
            <ul class="menu-speedbar">
                ...
                <li><a lang="en" href="dashboard.php"  lang="en">Dashboard</a></li>
    ...
    <div id="NewsArea" style="width:978px; margin:10px; border:1px solid #cacaca; padding-left:0px; padding-right:0px; display:table; border-radius:5px; border:none;">
        <div class="tabbable">
            <ul class="nav nav-tabs myPsClass">
                ...
                <li class="" style="width:170px;"><a href="#lB" data-toggle="tab"><i class="icon-sort-by-order" style="margin:0 auto; padding:0px; color:#22aeff;"></i> <span class="label label-info" style="-webkit-animation: glow 2s linear infinite;" lang="en">Recent EMR Activity</span></a></li>
            </ul>
...
$(document).ready(function() 
{
    ...
    console.log("TELEMED_TYPE: " + $("#TELEMED_TYPE").val() + " - IN_CONSULTATION: " + $("#IN_CONSULTATION").val());
    if($("#TELEMED_TYPE").val() == '1' && $("#IN_CONSULTATION").val() == '1')
    {
        $("#phone_consultation_text").text("You are on a phone consultation with member " + $("#phone_consultation_name").val() + ".");
...
//	    alert (id-1+' stopping '+(id-2)+' and '+(id));
	...
	//Start of code for EchoPendingReviews which periodically checks the any mails to be review by doctor sent by patients through         Send feature of UserDashboard.php file
    ...
    function EchoPendingReviews()
    {
        ...
        $.post("getPendingReviewMails.php", {Doctor: doctorId}, function(data, status)
		{
            ...
            for(var i = 0; i < items.length; i++)
            {
                ...
                html += '<button id="pending_review_mail'+i+'"  class="btn" title="Review" style="width:65%; color:grey; margin-bottom: 2px;">Review</button></td></tr>';
            ...
            //console.log('hi'+items[0].USER);
            for(var i = 0; i < items.length; i++)
            {
                  //Start of code for Review button in Pending Review division

                    $("#pending_review_mail"+i).on('click',function(data,success)
                        {
                            var doctorId = $("#MEDID").val();
                            var string = $(this).attr('id');
                            var index = string.charAt(string.length - 1);
                            console.log('Index is'+index);
                           // console.log(index.parseInt(index));
                            
                            console.log(doctorId);
                           // console.log(items[i]);
                            $.post("SetNewMailsAsRead.php",{doctorId: doctorId,userId: items[index].USER,messageId: items[index].MESSAGE_ID},function(data,success)
                                   {
                                      alert(data);
                                   });
                            ...
                            window.location = "patientdetailMED-new.php?IdUsu="+items[index].USER+"&Acceso="+'23432';
    ...
    */
	...
	function FillEngage()
	{
        ...
        $.post("GetEngagedLight.php", {Doctor: queMED, NReports: 3, Group: group}, function(data, status)
		{
            ...
            $('#Engage2T2').html('<span lang="en">ACTIVE</span>');
            load_next();
        });
    ...
    function GetPendingReferrals()
    {
        ...
        $.post("getPendingReferralsLight.php", {Doctor: queMED}, function(data, status)
		{
            ...
            for(var i = 0; i < items.length; i++)
            {
                ...
                html += '<td style="width: 20%; padding:6px; ">';
                html += '<a id="pending_referral_button" href="javascript:void(0)" class="btn" title="Acknowledge" style="width:65%; color:grey; margin-bottom: 2px;" lang="en"><i id="' + items[i].ID + '" class="icon-ok"></i> Acknowledge</a></td></tr>';
            }
            html += '</tbody>';

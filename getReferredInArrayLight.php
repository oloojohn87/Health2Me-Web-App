...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($n < $count-1)
    {
        $tr_style = "height:30px; border-bottom:1px solid #cacaca;";
    }
    else 
    {    
    ...
    $html .= '<td style="padding: 6px;"><a href="patientdetailMED-new.php?IdUsu='.$IdPac.'" style="text-decoration:none;"><div class="truncate" style="line-height:1; color: #54bc00; font-size:14px;">'.BeauStr($SurnameP).', '.strtoupper($NameP[0]).' '.$Iteration.'</div><div class="truncate" style="color: #22aeff; font-size:12px; margin-top:0px;"> '.strtoupper($SurnameD2).'</div></a></td>';
    $St1='OFF';	
    $St2='OFF';	
    $St3='OFF';	
    $St4='OFF';	

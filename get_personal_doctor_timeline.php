...
getDoctorViews($items, $pat_id, $doc_id, $user_id, $con, $pass);
getUploads($items, $pat_id, $doc_id, $user_id, $con, $pass);
...
//getSignUp($items, $pat_id, $doc_id, $user_id, $con);
...
function getDoctorViews(&$items, $pat_id, $doc_id, $user_id, &$con, $pass)
{
    $idpins = array();
    $idpins_ = array();
    $raw_images = array();
    ...
    while($row = $reports->fetch(PDO::FETCH_ASSOC))
    {
        if(in_array($row['IDPIN'], $idpins_))
        {
            ...
            if(!file_exists("temp/".$user_id."/PackagesTH_Encrypted/".$image))
            {
                ...
                {
                    $items[($row['DateTimeSTAMP'])]["File"][0] = 'NF'; // No File
                }
            }
...
function getUploads(&$items, $pat_id, $doc_id, $user_id, &$con, $pass)
{
...
function getSignUp(&$items, $pat_id, $doc_id, $user_id, &$con)
{
    // obtain the sign up date for this patient
    $signUp = $con->prepare("SELECT signUpDate FROM usuarios WHERE Identif = ?");
    $signUp->bindValue(1, $pat_id, PDO::PARAM_INT);
    $signUp->execute();
    $row = $signUp->fetch(PDO::FETCH_ASSOC);
    $signUpDate = $row['signUpDate'];
    $items[$signUpDate] = array("Type" => 'SignUp');
}
?>

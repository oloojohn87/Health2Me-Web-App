...
//and IdMed NOT IN (Select idmed from doctorslinkdoctors where idmed2='$MedID' and IdPac='$queUsu'))";*/
	...
	if($isDoctor == 1)
    {
        ...
        ? and IdPac=
        ? and estado=2))) order by fecha DESC");
...
if($EntryTypegroup==0)
{
	$query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 ORDER BY Fecha ASC ");
...
//$result = mysql_query("SELECT * FROM LifePin WHERE IdUsu='$queUsu' and emr_old=0 ORDER BY Fecha ASC ".$limitString);
...
if ($EntryTypegroup == $TipoAgrup[$row['Tipo']] || $EntryTypegroup == 0 || $EntryTypegroup == 5 )
{
	...
	if($EntryTypegroup == 5)
	{
		...
		$extensionR2 = strtolower(substr($row['RawImage'],strlen($row['RawImage'])-3,3));
	...
	if(!in_array($row['IdPin'], $blindReportId)){
		...
		//echo "*********************************".$extensionR;
		  if($extensionR=='wav'){
			...
			}elseif ($extensionR=='pdf' or $extensionR=='png')
				{
				$isPDF=1;
				...
				}elseif($extensionR=='jpg'|| $extensionR=='JPG')
			{
				decrypt_files($row['RawImage'],$MedID,$pass);
...
function decrypt_files($rawimage,$queMed,$pass )
{
	...
	*/
	...
	{
		$out = shell_exec("echo '".$pass."' | openssl aes-256-cbc -pass stdin -d -in ".$local."PackagesTH_Encrypted/".$ImageRaiz.".".$extension." -out ".$local."temp/".$ImageRaiz.".".$extension);
		//die($out.' '.$ImageRaiz);
	}

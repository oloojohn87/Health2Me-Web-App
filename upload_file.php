	...
	//for ($i = 0; $i < $size; $i++) {
				    ...
				    //echo md5_file($filename);
				        if(strtolower($extension)=="doc"){
                            ...
                            $extensions_list = array('png','TIFF', 'jpg', 'tiff', 'gif', 'mov', 'pdf');
                    ...
                    $findRefDocs = $con->prepare('SELECT personal_doctor FROM usuarios WHERE Identif = ?');
                    $findRefDocs->bindValue(1, $IdUsu, PDO::PARAM_INT);
                    $findRefDocs->execute();
                    $row_doc =$findRefDocs->fetch(PDO::FETCH_ASSOC);
                    
                    // doctor
                    if(isset($_SESSION['MEDID']))
                    {
                        //to the patient
                        $notifications->add('REPUPL', $_SESSION['MEDID'], true, $IdUsu, false, $IdUsu);
                    }
                    //below means patient himself uploading
                    else
                    {
                        //to the related doctors
                        $notifications->add('REPUPL', $IdUsu, false, $row_doc['personal_doctor'], true, $IdUsu);
                    }
                    ...
                    $IdPin = $con->lastInsertId();
						...
						if($extension=="jpg") {
                            ...
                            $new_image_nameTH = 'eML'.$confcode.'.TIFF';	
                        }else {

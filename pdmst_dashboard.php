...
if($user->doctor_pdmst_priv != 'pdmst' && $user->doctor_privilege != 3) $user->exit_onfail(5);
...
if($user->doctor_pdmst_priv == 'pdmst') {
                                $res = mysql_query("SELECT DISTINCT contract FROM consults");
                                echo '<option value="Global" selected lang="en">Global</option>';
                                echo '<option value="HTI_Global" lang="en">HTI Global</option>';
...
else if($user->doctor_privilege == 3) {
                                $res = mysql_query("SELECT DISTINCT contract FROM consults WHERE contract LIKE 'HTI%'");
                                echo '<option value="HTI_Global" selected lang="en">HTI Global</option>';
                                while($row = mysql_fetch_assoc($res))
                                {
                                    
                                    //if(strlen($row['contract']) > 0 && $row['contract'] != 'CATA') {
                                        echo '<option value="'.$row['contract'].'">'.$row['contract'].'</option>';
                                    //} else {
                                        //echo '<option value="NULL">H2M</option>';
                                    //}
...
<center><span lang='en'>Message to doctor</span></center>
    ...
    <script>
            ...
            function load_table(table, sort, ascending, page, scope, period, search)
            {
                ...
                {
                    console.log('scope: '+scope+', period: '+period+', sort_order: '+sort_order+', sortfield: '+sort+', search: '+search+', page: '+page);
                    ...
                    else if(table == 2) // consultations table
                    {
                        ...
                        for(var i = 0; i < info.length; i++)
                        {
                            ...
                            html += '<a href="patientdetailMED-new.php?IdUsu='+info[i]['Patient']+'" style="color: #5EB529">'+info[i]['patientName'] + ' ' + info[i]['patientSurname']+'</a>';
                            html += '</td><td style="background-color: '+color+'; width: 130px; height: 15px; text-align: center;">';
							...
							var resultxyz = doesFileExist('http://health2.me/recordings/'+info[i]['Recorded_File']);
                    ...
                    else if(table == 3) // doctors table
                    {
                        ...
                        {
                            var color = "#F5F5F5";
                            var docfullname = info[i]['name']+' '+ info[i]['surname'];
                            if(i % 2 == 1)
                            {
                                color = "#E9E9E9";
                            }
					...
					else if(table == 4) // doctors table
                    {
                        ...
                        for(var i = 0; i < info.length; i++)
                        {
                            ...
                            html += '<tr><td style="background-color: '+color+'; width: 225px; height: 15px; text-align: center;">';
                            html += '<a href="patientdetailMED-new.php?IdUsu='+info[i]['Identif']+'">'+info[i]['name']+' '+info[i]['surname']+'</a>';
                            html += '</td><td style="background-color: '+color+'; width: 225px; height: 15px; text-align: center;">';
...
console.log(d);
            ...
            $("#medical_availability_show_doctors").on('click', function()
            {
                if(medical_availability_show_doctors_value == 0)
                {
                    ...
                    if(langType == 'th') $(this).text('Ocultar todos los médicos');
                    else if(langType == "en") $(this).text("Hide All Doctors");
                    $("#all_doctors_availability").css("display", "block");
                }
                else
                {
                    medical_availability_show_doctors_value = 0;
                    if(langType == 'th') $(this).text('Mostrar todos los médicos');
                    else if(langType == "en") $(this).text("Show All Doctors");
			...
			var additional_licenses = '';
				...
				if(info[i]['additional_licenses'] == null){
				var additional_license_holder = 'No additional licenses';
				}else{
				var additional_license_holder = info[i]['additional_licenses'];
				}
			    additional_licenses = 'Additional Licenses : '+additional_license_holder;
                        }
                        else
                        {
                            id = info[i]['Identif'];
			...
			if(location == null || location == -1){
			    location = "Unknown";
                        ...
                        html += '<button id="search'+type_str+'_'+id+'_'+phone+'" style="width: 100%; height: 75px; background-color: #FBFBFB; color: #333; border: 0px solid #000; outline: none;text-align: left; border: 1px solid #E8E8E8;" title="'+additional_licenses+'">';
                        ...
                        html += info[i]['Name']+' '+info[i]['Surname']+'</span> <br/> <span style="font-size: 12px;">'+email+'</br>'+location+'</span>';
                        html += '</button>';
                    }
	    ...
	    var lang_var = 'en';
	    $("#select-language-sp").click(function(){
		lang_var = 'sp';
		$("#select-language-sp").attr('class','btn btn-success');
		$("#select-language-en").attr('class','btn btn-default');
	    });

	    $("#select-language-en").click(function(){
		lang_var = 'en';
		$("#select-language-sp").attr('class','btn btn-default');
		$("#select-language-en").attr('class','btn btn-success');
	    });

            $("#telemedicine_status").dialog({bigframe: true, width: 400, height: 150, autoOpen: false, close: function(){ clearInterval(status_interval); }});
            $("#launch_telemedicine_button").on('click', function()
            {
		$("#call_status_label").css("color", "#E07221");
                $("#call_status_label").text('Connecting...');
                $("#launch_telemedicine_button").attr("disabled", true);
		$("#telemedicine_status").dialog('open');
                
                $.post("start_telemed_phonecall.php?stage=init-call", {grant_access: 'Manually-Launched',pat_phone: selected_user_number, doc_phone: selected_doctor_number, doc_id: selected_doctor, pat_id: selected_user, doc_name: $("#selected_doctor_label").text(), pat_name: $("#selected_user_label").text(), override: true, message_text: $('#launch-consultation-message').val(), language:lang_var}, function(data, status)
                {
latest_sid = '';

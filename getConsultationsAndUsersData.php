...
/*if($scope == "Global")
{
    ...
    if($period == 1)
    {
       ...
       */
        /*for($i=0;$i<=23;$i++)
        {
            ...
            $query = $con->prepare("select consultationId from consults where DateTime between ? and ?");
            $query->bindValue(1, $curdate." ".$time.":00:00", PDO::PARAM_STR);
            $query->bindValue(2, $curdate." ".$time.":59:59", PDO::PARAM_STR);
            $query->execute();
            $result = $query->rowCount();
            
            $query1 = $con->prepare("select distinct(patient) from consults where DateTime between ? and ?");
        ...
        $query2 = $con->prepare("SELECT DISTINCT Patient FROM consults WHERE DateTime LIKE '".date("Y-m-d")."%'");
        $query2->execute();
    ...
    elseif($period == 2)
    {
       ...
       while(strcmp($current_date, $todayDate) != 0)
       {
        ...
        $query2 = $con->prepare("SELECT DISTINCT Patient FROM consults WHERE DateTime BETWEEN ? AND ?");
    ...
    elseif($period == 4)
    {
        ...
        while(strcmp($current_date, $todayDate) != 0)
        {
            ...
            $query = $con->prepare("select consultationId from consults where DateTime like ?");
            ...
            $query1 = $con->prepare("select distinct(patient) from consults where DateTime like ?");
...
{*/
    ...
    if($scope == 'HTI_Global')
    {
        $scope_str = " AND (c.contract LIKE 'HTI%' OR c.contract = 'Manual%20Launch')";
        $HTI_verify_query = "AND d.previlege = 3 ";
    }
    else if($scope == 'H2M')
    {
        $scope_str = " AND (c.contract='".$scope."' OR c.contract IS NULL)";
    }
    else if($scope != 'Global')
    {
        $scope_str = " AND c.contract='".$scope."'";
        if(strpos($scope, 'HTI') !== false) $HTI_verify_query = "AND d.previlege = 3 ";
    }
    

    /*$scope_str = " and c.contract = ?";
    if($scope == 'Global')
        $scope_str = " and c.contract LIKE 'HTI%'";*/
    if($period == 1)
    {
   ...
   */
        for($i=0;$i<=23;$i++)
        {
            ...
            $query = $con->prepare("select c.consultationId from consults c INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime between ? and ?".$scope_str);
            $query->bindValue(1, $curdate." ".$time.":00:00", PDO::PARAM_STR);
            $query->bindValue(2, $curdate." ".$time.":59:59", PDO::PARAM_STR);
            $query->execute();
            $result = $query->rowCount();
            
            $query1 = $con->prepare("select distinct(c.Patient) from consults c  INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime between ? AND ?".$scope_str);
            $query1->bindValue(1, $curdate." ".$time.":00:00", PDO::PARAM_STR);
            $query1->bindValue(2, $curdate." ".$time.":59:59", PDO::PARAM_STR);
            $query1->execute();
            $result1 = $query1->rowCount();           
        ...
        $query2 = $con->prepare("select distinct(c.Patient) from consults c INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime LIKE '".date("Y-m-d")."%'".$scope_str);
        if(strpos($scope, 'HTI') !== false)
    ...
    elseif($period == 2)
    {
        ...
        while(date($current_date) < date($todayDate))
        {
        ...
        $query2 = $con->prepare("select distinct(c.Patient) from consults c INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime BETWEEN ? AND ?".$scope_str);
        $query2->bindValue(1, $dateOfLastSunday, PDO::PARAM_STR);
        $query2->bindValue(2, $todayDate, PDO::PARAM_STR);
        $query2->execute();
        $numUsers = $query2->rowCount();
    ...
    elseif($period == 4)
    {
        ...
        while(date($current_date) < date($todayDate))
        {
            ...
            $query = $con->prepare("select c.consultationId from consults c INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime like ?".$scope_str);
            $query->bindValue(1, $compare_str."%", PDO::PARAM_STR);
            $query->execute();
            $result = $query->rowCount();
            array_push($yearlyConsultationData,$result);
            ...
            $query1 = $con->prepare("select distinct(c.Patient) from consults c INNER JOIN doctors d ON d.id = c.Doctor ".$HTI_verify_query."INNER JOIN usuarios u ON u.Identif = c.Patient where c.DateTime like ?".$scope_str);
            $query1->bindValue(1, $compare_str."%", PDO::PARAM_STR);
            $query1->execute();
            $result1 = $query1->rowCount();
            array_push($yearlyUserData,$result1);
...
elseif($period == 5)
    {
        ...
        $firstDateOfYear = date('1900-01-01');
        $todayDate = date("Y-m-01");
        $current_date = "";
        $count = 0;
...
elseif($period == 5)
{
 echo json_encode(array("consultations" => $totalConsultationData, "users" => $totalUserData, "numUsers" => $numUsers));
}
?>

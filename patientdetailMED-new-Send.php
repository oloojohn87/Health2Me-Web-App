...
if(isset($_GET['doctor_email']))
   {
       ...
       $query->bindValue(1,$_GET['doctor_email'],PDO::PARAM_STR);
       $query->execute();
       $result = $query->fetch(PDO::FETCH_ASSOC);
       $doctorIdSend = $result['id'];
   }
   
$telemed_on = false;
$telemed_value=0;
$doctorEmail = $_GET['doctor_email']; //Statement added by Pallab
...
$query = $con->prepare("select * from temporary_doctor_access where doctorId =? and patientId = ? and expirationTime > now()");
$query->bindValue(1,$doctorIdSend,PDO::PARAM_INT);
...
if(isset($_SESSION['MEDID']))
{
    ...
    $results->bindValue(1, $_SESSION['MEDID'], PDO::PARAM_INT);
...
if($countOfDoctor == 0 && $countOfDoctor2 == 0) 
{  
    header("location:initialLogin.php?InvalidAccess=".'InvalidUser');
}
...
if($MedID >= 0)
{
    ...
    } */ // End of commented out by Pallab for Non h2M doctor for Send
...
<html lang="en"><head>
    ...
    <link href="css/toggle-switch.css">
...
<!-- Quick-Start: Step 3 -->
 ...
 <!-- New Style Added by Pallab for create account button -->   
    <style>
.bottom_buttons{
    width: 148px;
    height: 29px;
...
<!--  <body onload="$('.note').trigger('click'); $('.TABES').children().trigger('click');"> -->
  <body onload=" $('.TABES:eq(<?php echo $tabToOpen;?>)').click();">
      ...
      <input type="hidden" id="entry_doctorid" value="<?php echo $doctorIdSend; ?>" />
          ...
          <?php
		   ...
		   <div class="btn-group pull-right" >
            ...
            <a class="btn btn-profile dropdown-toggle" id="button-profile" data-toggle="dropdown" href="#">
              <span class="name-user"><strong lang="en">Welcome, Guest</strong></span>
            ...
            <ul class="clear_ul" >
           <!-- Commented by Pallab  <li><a href="signin.html" lang="en"><i class="icon-globe"></i> Home</a></li> -->
              
             <!-- <li><a href="medicalConfiguration.php"><i class="icon-cog"></i> Settings</a></li>-->
             <li><a href="logout.php" lang="en"><i class="icon-off"></i> Close</a></li> <!-- Changed from i> Sign Out< to Close -->
            </ul>
            </div>
          </div>
          <!--Button User END-->  
         ...
         <div class="modal-body" style="text-align: center;font: 10pt Arial, sans-serif; background: lightgrey;">
			<center>
				<table style="background:transparent; height:150px;" >
                    ...
                    <tr>    
						<td style="height:24px;"><textarea rows="5" cols="150" style="width:420px;resize:none" id="recom_text">	</textarea></td>
					</tr>
				</table>
			</center>
		 </div>
     ...
     <div class="speedbar-content">
		...
		<?php
           ...
           if(isset($_SESSION['MEDID']))
            ...
            echo '<ul class="menu-speedbar">';
            echo '<button id ="CreateAccount" class="bottom_buttons" style="background-color: #54bc00;">Create Account</button>';
            echo '</ul>';
        ...
        <div class="clearfix" style="margin-bottom:20px;"></div>
		 ...
		 <!-- image upload changes start -->
		  ...
		  ?>	
			...
			<!--	
Commented out by Pallab for Send as not required by Non H2M doctor 
<table style="margin-left:10px;background-color:transparent;">
				...
				<td style="height:100px;"> -->
            <!-- BELOW LINE UNCOMMENTED BY PALLANB AS REQUESTED BY CARLOS TO GET THE NAME OF THE PATIENT TO SHOW TO NON H2M DOCTOR -->
...
<span id="email" style="font-size: 14px; color: #5EB529; font-weight: bold; font-family: Arial, Helvetica, sans-serif; margin-left:10px;"><?php echo $email;?></span>
	...
	<!-- image upload changes end-->
        ...
        <!--Commented out by Pallab as not required for Non H2M doctor for Send -->
...
Start of commmenting by Pallab as it is not required for Non H2M doctor for Send 
...
{echo '<input type="hidden" id="referral_state" value="0">';} ?>
     ...
     <!--    <!--Upload Box Start-->
     <!--  Commented out by Pallab for Send as not required for non h2m doctor <div class="label label-info" id="EtiTML" style=" margin-top: 0px; width: 96%; font-size:16px; text-shadow:none; text-decoration:none; padding: 7px; text-align: center;">Toolbox</div> -->
  ...
  <div id="createAccount" title = "Create Account" style="overflow:visible;display:none;">
        <div style="border:solid 1px #cacaca; margin-top:5px; padding:10px;">
			...
			<table style="width:100%;background-color:white">
                ...
                <tr>
					...
					<td style="width:80%"><select id = "Sex" style="margin-top:12px;">
        ...
        <a id="createAccountNewDoc" class ="btn btn-success" style="width:150px; color:#FFF; float:right; margin-top:25px;margin-bottom:10px;"><span>Create Account</span></a>
...
<!-- End of code for modal window for create account button -->
  ...
  <div id="tabsWithStyle" style="margin-top:20px" class="style-tabs"> 
          ...
          <ul id="myTab" class="nav nav-tabs tabs-main" style="width: 972px;">
            ...
            <li id="8" class="TABES" style="text-align: center; width:<?php if($multireferral>0) {echo 97;} else {echo 108;}?>px; background-color:<?php echo $TipoColorGroup[8] ?>;"><a href="#ALL" lang="en" data-toggle="tab" style="height:40px; color:<?php echo $TipoColorGroup[8] ?>;"><i class="<?php echo $TipoIconGroup[8] ?> icon-large" style="color:RGB(111,111,111); font-size: 1.0em; width:100%;"></i><div style="width:100%"></div>Pat. Notes</a></li>
  ...
  <!--  <script type="text/javascript" src="http://code.jquery.com/jquery-1.8.0.min.js"></script>--->
    ...
    $(document).ready(function() {
        ...
        var queUrl ='SetUserActivity.php?doctorid=<?php echo $doctorIdSend;?>&userid=<?php echo $USERID;?>';		
        result = LanzaAjax(queUrl);
	...
	$('#NewMES').live('click',function(){
		var queUrl ='getInboxMessageUNREAD.php?IdMED=<?php echo $MedID;?>&patient=<?php echo $USERID;?>&sendingdoc='+otherdocid[i];		
				...
				MesTimeline = MesTimeline + '<div style="float:left; width:87%; margin-left:3%; border-bottom:thin dotted #cacaca;">'; 
					...
					if (NumReports>0)
					{
						MesTimeline = MesTimeline + '<div style="margin:0 auto; width:95%; border:solid #cacaca; border-radius:5px; height:80px;">'; 
							...
							while (rn < NumReports)
							{
								var cadena = 'DecryptFileId.php?reportid='+splitted[rn]+'&queMed='+<?php echo $MedID;?>;
								var RecTipo = LanzaAjax (cadena);
	...
	$(".ThumbTwitt").live('click',function() {
		 var myClass = $(this).attr("id");
		 var cadena = 'DecryptFileId.php?reportid='+myClass	+'&queMed='+<?php echo $MedID;?>;
		 var RecTipo = LanzaAjax (cadena);
		 //var thumbnail = RecTipo.substr(0,RecTipo.indexOf("."))+'.png';
 ...
 $('#PrintImage').live('click',function(){
	...
	$("#EvolutionToggle").live('click',function() {
		...
		if(fileFound)
		{
			...
			var cadena = 'DecryptFile.php?rawimage='+rawimage+'&queMed='+<?php echo $MedID;?>;
			var RecTipo = LanzaAjax (cadena);
			
			var filepath='temp/<?php echo $MedID ;?>/Packages_Encrypted/'+rawimage;
   ...
   $(".TABES").live('click',function() {
        ...
        var idpins = '';
        <?php if(isset($_GET['idpins'])){ echo 'idpins = "&idpins='.$_GET['idpins'].'";';} ?>
		...
		var queUrl = 'getNumThumbnails-Send.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+idpins;
		//console.log(queUrl);
		num_reports = LanzaAjax (queUrl);
        console.log(queUrl);
		//alert(num_reports);
        console.log(idpins+" "+num_reports);
		...
		//var queUrl ='CreateReportStream.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID;
		var queUrl ='CreateReportStream-Send.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&num_reports='+num_reports+idpins;
		//console.log(queUrl);
				...
				$.ajax(
				   ...
				   success: function(data) {
							if (typeof data == "string") {
                                        ...
                                        var files_to_delete = '';
                                        $(".queIMG").each(function()
                                          {
                                              
                                              parser.href = $(this).children().eq(0).attr("src");
                                              if(files_to_delete.length > 0)
                                                  files_to_delete += '_';
                                              files_to_delete += parser.pathname.substr(1);
                                              //console.log(parser.pathname.substr(1));
                                          });
                                        console.log(files_to_delete);
                                        $.post("deleteFile.php", {file: files_to_delete});
   ...
   $("#StreamContainerALL").scroll(function() {
	...
	function get_more_reports(queid,offset)
	{
        ...
        var idpins = '';
        <?php if(isset($_GET['idpins'])){ echo 'idpins = "&idpins='.$_GET['idpins'].'";';} ?>
		var queUrl ='CreateReportStreamChunk-Send.php?ElementDOM=na&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+'&offset='+offset+'&jump='+jump+idpins;
		//alert(queUrl);
		var RecTipo='<span class="label label-success" style="left:0px; margin-left:0px; margin-top:20px; margin-bottom:20px; font-size:14px; text-shadow:none; text-decoration:none;background-color:red">Could not load Reports due to Internet issues</span>';
		//$("#H2M_Spin_Stream").show();
    ...
    $("#BotonTestRS").live('click',function() {
        ...
        var idpins = '';
        <?php if(isset($_GET['idpins'])){ echo 'idpins = "&idpins='.$_GET['idpins'].'";';} ?>
		var queUrl ='CreateReportStream-Send.php?ElementDOM='+ElementDOM+'&EntryTypegroup='+EntryTypegroup+'&Usuario='+Usuario+'&MedID='+MedID+idpins;
            ...
            if(clicks === 1) {
                ...
                window.location.href="#AreaConten";
                   ...
                   timer = setTimeout(function() {
                    ...
                    {
                        var cadena = 'DecryptFile_PDMST.php?rawimage='+queImg+'&queMed='+doc_id;
                        var RecTipo = LanzaAjax (cadena);
                        $.post("deleteFile.php", {file: 'temp/'+queImg});
                        console.log(cadena);
                
                        var contenTHURL = 'temp/'+ImagenRaiz+'.png';  // PONER /PinImageSetTH/ en lugar de /PinImageSet/ PARA CARGAR THUMBNAILS EN LUGAR DE IMAGENES COMPLETAS
                        var contenURL =   'https://www.health2.me/live/v1_0_1/temp/'+ImagenRaiz+'.'+extensionR;  // PONER /PinImageSetTH/ en lugar de /PinImageSet/ PARA CARGAR THUMBNAILS EN LUGAR DE IMAGENES COMPLETAS
                    ...
                    else if(extensionR=='MOV')
                    {
                        ...
                        var src = 'temp/'+ImagenRaiz+'.'+extensionR;
                        //alert(src);
                    ...
                    else if(extensionR=='jpg' || extensionR=='jpeg' || extensionR=='JPG' )
                    {
                        ...
                        var RecTipo = LanzaAjax (cadena);
                        $.post("deleteFile.php", {file: 'temp/'+queImg});
                        var contenTHURL = 'temp/'+queImg;  // PONER /PinImageSetTH/ en lugar de /PinImageSet/ PARA CARGAR THUMBNAILS EN LUGAR DE IMAGENES COMPLETAS
                        var contenURL =   'temp/'+queImg;  // PONER /PinImageSetTH/ en lugar de /PinImageSet/ PARA CARGAR THUMBNAILS EN LUGAR DE IMAGENES COMPLETAS
                        //var conten = '<img id="ImagenN" src="'+contenURL+'" alt="">';
                        ...
                        if(canal == '7' ||extensionR=='png'||extensionR=='PNG'){
                            ...
                            var RecTipo = LanzaAjax (cadena);
                            $.post("deleteFile.php", {file: 'temp/'+queImg});
                            var contenTHURL = 'temp/'+queImg; 
                            var conten = '<img id="ImagenN" src="temp/'+queImg+'" alt="'+queId+'" style="border:1px solid #666CCC; margin:0 auto; display:block;">';
                                //alert('here');
                        }else{
                        if (subtipo=='XX') { 
                    ...
                    var contenTH = '<img id="ImagenTH"  src="images/deletedfile.png" alt="">';
...
//Adding button action for blind reports
 ...
 //Adding changes for the send button 
		 ...
		 setTimeout(function(){
		   ...
		   $("[id^='newinbox']").live('click',function(){
			  ...
			  /* alert(queUrl);
			   $("#datatable_3_"+i).load(queUrl, function( response, status, xhr ) {  
				...
				whichdocidclicked=i;
				//alert (i);
				var queUrl ='getInboxMessage.php?IdMED=<?php echo $MedID;?>&patient=<?php echo $USERID;?>&sendingdoc='+otherdocid[i];
				var RecTipo=LanzaAjax(queUrl);
				//alert (RecTipo);
		  ...
		  $("[id^='newoutbox']").live('click',function(){
			  //alert('trigger');
				...
				whichdocidclicked=i;
				//alert (i);
			 var queUrl ='getOutboxMessage.php?IdMED=<?php echo $MedID;?>&patient=<?php echo $USERID;?>&receivingdoc='+otherdocid[i];
  ...
  $("[id^='delete_message']").live('click',function(){
   ...
   var cadena ='getNotificationCount.php?IdMED=<?php echo $MedID;?>';
   ...
   if(notify_num){
	...
	//var notify_num=78;
	var queUrl ='getNotificationMessages.php?IdMED=<?php echo $MedID;?>';
    //
 ...
 */ 
  ...
  function getMedCreator(UserId) {
   ...
   function getReportCreator(Idpin,Iddoc,Idusu) {
 	...
 	var cadenaGUD = 'getReportCreator.php?Idpin='+Idpin+'&Iddoc=<?php echo $MedID;?>&Idusu=<?php echo $IdUsu;?>';
	//alert(cadenaGUD);
    $.ajax(
           {
           url: cadenaGUD,
...
function urlExists(url, callback){
  $.ajax({
	...
	function next_click()
	{
		...
		{
			var cadena = 'DecryptFile.php?rawimage='+file_name+'&queMed='+<?php echo $MedID;?>;
			//alert(cadena);
			var RecTipo = LanzaAjax (cadena);
			//alert(RecTipo);
            var doctorId = -1;
	...
	function LanzaAjax (DirURL)
        ...
        //Code for popping up the modal window for Create Account button
            $("#CreateAccount").on('click',function(){
                $("#EmailCreateAccountPage").removeAttr('disabled');
                $("#EmailCreateAccountPage").val('<?php echo $_GET['doctor_email']; ?>');
                $("#EmailCreateAccountPage").attr('disabled', 'disabled');
                $("#createAccount").dialog({bigframe: true, width: 550, height: 400, modal: false});
                $(".ui-dialog .ui-dialog-titlebar").css("height", "40px");
              }); 
        
        
        //Start of code for createAccountNewDoc(the button in the create account modal window) button

              var name,surname,email,password,repeatPassword,date,gender;
              var emailDocToBeAdded = '<?php echo $doctorEmail; ?>';
        
             $("#createAccountNewDoc").on('click',function()
            {
                    
                    gender = $( "#Sex option:selected" ).text();
                    ...
                    if(passwordStatus == 'correct')
                    {
                        ...
                        $.get("createAccountNewDoc.php?name="+name+"&surname="+surname+"&email="+email+"&password="+password+"&date="+date+"&gender="+gender+"&emailDoc="+emailDocToBeAdded+"&memid=<?php echo $_GET['IdUsu'] ?>&pins=<?php echo $_GET['idpins'] ?>",function(data,status)
                        {

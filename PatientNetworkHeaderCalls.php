...
// Connect to server and select databse.
    ...
    (SELECT patientID AS pat, BIT_OR(scheduledEndDate >= SYSDATE() AND doctorPermission = 1 AND patientPermission = 1) AS probe_on, BIT_OR(scheduledEndDate > SYSDATE() AND (creationDate BETWEEN SYSDATE()-INTERVAL 30 DAY AND SYSDATE())) AS probe_recent FROM probe WHERE doctorID = ? GROUP BY patientID) AS PROBE ON PROBE.pat = idus.Identif

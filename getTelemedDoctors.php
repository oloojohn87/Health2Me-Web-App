...
$location = '';
$hourly_rate = 0;
$tag = 0; // used to identify search result
...
if(isset($_POST['personal_doctor_search']))
{
    $personal_doctor_search = $_POST['personal_doctor_search'];
}
if(isset($_POST['tag']))
{
    $tag = $_POST['tag'];
}
if(isset($_POST['grant_access']) && !empty($_POST['grant_access'])){
	$grant_access = 'grant_access = "'.$_POST['grant_access'].'" AND';
}else{
	$grant_access = '';
}
...
$next_spot = 1;
$phs_spot = 0;
$query = "SELECT * FROM doctors WHERE ".$grant_access." IdMEDEmail is not null AND Name NOT LIKE '% %' AND Surname NOT LIKE '% %' AND Name != '' AND Surname != ''";
if($telemed_only == 1)
{
    $query .= 'AND telemed = 1 ';
}
...
if($personal_doctor_search == 1)
{
    $pat_identifier = $con->prepare("SELECT phs_identifier FROM usuarios WHERE Identif = ?");
    $pat_identifier->bindValue(1, $_POST['pat_id'], PDO::PARAM_INT);
    $pat_identifier->execute();
    $pat_identifier_row = $pat_identifier->fetch(PDO::FETCH_ASSOC);
    if($pat_identifier_row['phs_identifier'] == NULL)
    {
        $query .= "AND phs_identifier = '-1' ";
    }
    else
    {
        $query .= "AND phs_identifier = ? ";
        $phs_spot = $next_spot;
        $next_spot += 1;
        $phs = $pat_identifier_row['phs_identifier'];
    }
}
if($order == 0) // name
{
...
if(strlen($phs) > 0)
{
    $result->bindValue($phs_spot, $phs, PDO::PARAM_STR);
}
...
echo json_encode(array("data" => $data, "next" => $next, "next_result" => $starting_point + 1, "tag" => $tag));

...
if (!$con)
 ...
 $query1 = $con->prepare("select email,Alias from usuarios where Identif = ?");
 ...
 echo $emailUser;
 $Content = file_get_contents($hardcode'.templates/RequestReportsForNonH2M.html');
...
$query2 = $con->prepare("select count(*) as count from doctors where IdMEDEmail = ? and salt is null");
...
if(($count1 == 0 || $count1 == 1) && $count2 == 0)
{
        // If the doctor is not into the doctors table then insert him into the doctors table
         if($count1 == 0)
          {
             $query = $con->prepare("insert into doctors (IdMEDEmail,previlege) values(?,2)");

...
if($EntryTypegroup==0)
{
	$query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 ORDER BY Fecha ASC ".$limitString);
	$query->bindValue(1, $queUsu, PDO::PARAM_INT);
}
else
{
	$query=$con->prepare("SELECT * FROM lifepin WHERE IdUsu=? and emr_old=0 ".$limitString." ORDER BY Fecha ASC LIMIT ".$limit);
	$query->bindValue(1, $queUsu, PDO::PARAM_INT);
}
